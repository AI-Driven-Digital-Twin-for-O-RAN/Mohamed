...
// handover_xapp.csv: time_sec,ue_id,from_cell,to_cell,event,executed_ok (event=COMMAND_SENT|COMPLETED)
static FILE* g_handover_xapp_log = NULL;
static int g_handover_xapp_header_done = 0;

static void log_handover_xapp(uint16_t ue_id, uint16_t from_cell, uint16_t to_cell, const char* event, int executed_ok) {
    if (g_handover_xapp_log == NULL) {
        const char* home = getenv("HOME");
        char path[512];
        if (home && home[0])
            snprintf(path, sizeof(path), "%s/handover_xapp.csv", home);
        else
            snprintf(path, sizeof(path), "handover_xapp.csv");
        g_handover_xapp_log = fopen(path, "a");
        if (g_handover_xapp_log == NULL) return;
    }
    if (!g_handover_xapp_header_done) {
        fprintf(g_handover_xapp_log, "time_sec,ue_id,from_cell,to_cell,event,executed_ok\n");
        g_handover_xapp_header_done = 1;
    }
    time_t now = time(NULL);
    fprintf(g_handover_xapp_log, "%ld,%u,%u,%u,%s,%d\n", (long)now, ue_id, from_cell, to_cell, event, executed_ok);
...
uint16_t doHandoverAction(callback_data_t data)
{
    ...
    printf("\n" CLR_BCYAN
           "  ╔═════════════════════════════════════════════════════════════╗\n"
           "  ║  ▶  HANDOVER COMMAND  #%-3d                                 ║\n"
           "  ║     UE " CLR_BYELLOW "%-4d" CLR_BCYAN "  |  Cell " CLR_BYELLOW "%-2d"
           CLR_BCYAN " ──▶ Cell " CLR_BGREEN "%-2d" CLR_BCYAN
    ...
    /* BROADCAST to ALL nodes: ns3 mmwave-enb receives control only on the socket
     * that has the UE. Node ordering may not match cell IDs. Broadcasting ensures
     * the control reaches the mmwave eNB that actually has the UE. Each eNB
     * checks m_rrc->GetUeMap() and ignores if UE not found. */
    ...
    for (size_t i = 0; i < (*data.nodes).len; ++i) {
        ...
        if (ans.success) {
            ...
            LOG_OK("RC command delivered to node " CLR_BWHITE "%zu" CLR_RESET, i);
        }
    }
    ...
    if (handover_sent) {
        ...
        LOG_ERR("Status: " CLR_BRED "FAILED" CLR_RESET " — no RC command delivered to any node");
        ctx->state = HO_STATE_IDLE;
        ue->ho_context = *ctx;
        return 0;
    }
...
int main(int argc, char* argv[]) 
{
  ...
  printf("\n" CLR_BBLUE
    ...
    "  ║  " CLR_CYAN "Broadcast RC       " CLR_BWHITE "All connected E2 nodes" CLR_BBLUE "              ║\n"
    "  ║                                                          ║\n"
    "  ╚══════════════════════════════════════════════════════════╝\n"
    CLR_RESET "\n",
