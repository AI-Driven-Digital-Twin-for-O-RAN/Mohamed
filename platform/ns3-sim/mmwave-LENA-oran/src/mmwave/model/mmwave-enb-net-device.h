...
//class MmWavePhy;
      ...
      class MmWaveEnbNetDevice : public MmWaveNetDevice {
        public:
            ...
            // L3 UE SINR trace (for LSTM CSV and external listeners)
            typedef TracedCallback<uint64_t,   // IMSI
                                   uint16_t,  // servingCellId
                                   double,    // servingSinrDb
                                   uint16_t,  // bestNeighCellId
                                   double>    // bestNeighSinrDb
                L3ReportUeSinrTracedCallback;
        ...
        private:
            ...
            // Compute RSRP for all cells by iterating NodeList gNBs (correct per-cell positions)
            void ComputeRsrpForAllCells (uint64_t imsi, double &servingRsrp,
                                         std::vector<std::pair<int, double>> &neighRsrpVec);

            // Periodic L3 SINR reporting for CSV/logging (used when m_sendCuCp == true)
            void FireL3ReportUeSinrForCsv(void);

            std::map <uint64_t, std::map<uint16_t, long double>> m_l3sinrMap;
            // Real RSRP map: imsi → cellId → rsrp_dBm
            // Populated in RegisterNewSinrReading() using 3GPP UMi path loss
            std::map <uint64_t, std::map<uint16_t, double>> m_l3rsrpMap;
            // Periodic CSV/L3 SINR event
            EventId m_csvTraceEventId;
            // Trace source instance
            L3ReportUeSinrTracedCallback m_l3ReportUeSinrTrace;
