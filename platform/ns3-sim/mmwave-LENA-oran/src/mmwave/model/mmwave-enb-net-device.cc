...
#include <ns3/node-list.h>          // NodeList::Begin/End for UE position lookup
  ...
  static TypeId tid =
      TypeId ("ns3::MmWaveEnbNetDevice")
          ...
          .AddAttribute ("RC_E2functionID", "Function ID to subscribe", DoubleValue (3),
                         ...
                         MakeDoubleChecker<double> ())
          .AddTraceSource ("L3ReportUeSinr",
                           "Fired when L3 report is built for a UE: imsi, servingCellId, servingSinrDb, bestNeighCellId, bestNeighSinrDb (for LSTM CSV).",
                           MakeTraceSourceAccessor (&MmWaveEnbNetDevice::m_l3ReportUeSinrTrace),
                           "ns3::MmWaveEnbNetDevice::L3ReportUeSinrTracedCallback");
  return tid;
}
  ...
  if (m_sendCuCp == true)
    {
      ...
      // Periodic L3 SINR trace for LSTM CSV (even without E2 subscription)
      m_csvTraceEventId = Simulator::Schedule (Seconds (0.5), &MmWaveEnbNetDevice::FireL3ReportUeSinrForCsv, this);
    }
}
  ...
  if (imsiFound)
    {
      ...
      // ── Compute real RSRP using 3GPP TR 38.901 UMi LoS path loss ─────────
      // RSRP (dBm) = TxPower_dBm − PathLoss_dB
      // PL = 32.4 + 21·log10(d3D_m) + 20·log10(fc_GHz)   [UMi LoS, Eq.7.4.1-1]
      // This gives the Reference Signal Received Power as seen by the UE
      double rsrp_dBm = -200.0;   // sentinel: unknown

      // gNB position — this device's node
      Ptr<MobilityModel> enbMob = GetNode ()->GetObject<MobilityModel> ();
      if (enbMob != nullptr)
        {
          Vector enbPos = enbMob->GetPosition ();

          // Search NodeList for UE whose MmWaveUeNetDevice has this IMSI
              ...
              for (uint32_t d = 0; d < node->GetNDevices (); d++)
                {
                  Ptr<MmWaveUeNetDevice> ueDev =
                      node->GetDevice (d)->GetObject<MmWaveUeNetDevice> ();
                  if (ueDev != nullptr && ueDev->GetImsi () == imsi)
                    {
                      Ptr<MobilityModel> ueMob = node->GetObject<MobilityModel> ();
                      if (ueMob != nullptr)
                        {
                          ...
                          if (d3d < 1.0) d3d = 1.0;   // minimum distance

                          // 3GPP UMi LoS path loss (TR 38.901, Table 7.4.1-1)
                          double fc_ghz = 3.5;          // CenterFrequency default
                          double tx_dbm = GetPhy ()->GetTxPower ();
                          double pl = 32.4 + 21.0 * std::log10 (d3d)
                                           + 20.0 * std::log10 (fc_ghz);
                          rsrp_dBm = tx_dbm - pl;
                          NS_LOG_LOGIC ("RSRP UE " << imsi << " Cell " << cellId
                                        << " d3D=" << d3d << "m tx=" << tx_dbm
                                        << " PL=" << pl << " RSRP=" << rsrp_dBm << " dBm");
                        }
                      goto rsrp_found;
                    }
                }
      ...
      m_l3rsrpMap[imsi][cellId] = rsrp_dBm;
    }
}
...
{
  fprintf(stderr, "[RC-HO] ControlMessageReceivedCallback ENTERED cellId=%u\n", (unsigned)m_cellId);
        ...
        switch (controlMessage->m_e2SmRcControlHeaderFormat1->ric_ControlAction_ID)
          {
            case RicControlMessage::Connected_Mode_Mobility_Control_Action_ID::Handover_Control: {
              ...
              fprintf(stderr, "[RC-HO] Cell %u received HO: IMSI %lu -> target %u\n", (unsigned)m_cellId, (unsigned long)imsi, (unsigned)targetCellId);
              NS_LOG_UNCOND("[RC-HO] Cell " << m_cellId << " received HO cmd: IMSI " << imsi << " -> target " << targetCellId);
              ...
              if (!ueFound)
              {
                NS_LOG_UNCOND("[RC-HO] UE " << imsi << " NOT FOUND in cell " << m_cellId << " - skipping");
                return;
              }
    
              NS_LOG_UNCOND("[RC-HO] Cell " << m_cellId << " processing handover UE " << imsi << " -> cell " << targetCellId);    
...
void
MmWaveEnbNetDevice::FireL3ReportUeSinrForCsv (void)
{
  auto ueMap = m_rrc->GetUeMap ();
  for (auto ue : ueMap)
    {
      uint64_t imsi = ue.second->GetImsi ();
      if (m_l3sinrMap.find (imsi) == m_l3sinrMap.end ())
        continue;
      if (m_l3sinrMap[imsi].find (m_cellId) == m_l3sinrMap[imsi].end ())
        continue;
double rawSinr = m_l3sinrMap[imsi][m_cellId];
if (rawSinr <= 0) continue;
double sinrThisCell = 10 * std::log10 (rawSinr);
      uint16_t bestNeighCellId = 0;
      double bestNeighSinrDb = -999.0;
      ...
      for (std::map<long double, uint16_t>::iterator it = --sortFlipMap.end ();
        ...
        {
          if (it->second != (uint16_t)m_cellId && bestNeighCellId == 0)
            {
              bestNeighCellId = it->second;
              bestNeighSinrDb = 10 * std::log10 (it->first);
              break;
            }
          itIndex++;
        }
      m_l3ReportUeSinrTrace (imsi, m_cellId, sinrThisCell, bestNeighCellId, bestNeighSinrDb);
    }
  m_csvTraceEventId = Simulator::Schedule (Seconds (m_e2Periodicity), &MmWaveEnbNetDevice::FireL3ReportUeSinrForCsv, this);
}

void
MmWaveEnbNetDevice::ComputeRsrpForAllCells (uint64_t imsi, double &servingRsrp,
                                             std::vector<std::pair<int, double>> &neighRsrpVec)
{
  servingRsrp = -200.0;
  neighRsrpVec.clear ();

  // Find UE position — try McUeNetDevice first (MC scenario), fall back to MmWaveUeNetDevice
  Vector uePos;
  bool ueFound = false;
  for (auto nit = NodeList::Begin (); nit != NodeList::End () && !ueFound; ++nit)
    {
      Ptr<Node> node = *nit;
      for (uint32_t d = 0; d < node->GetNDevices () && !ueFound; d++)
        {
          Ptr<NetDevice> dev = node->GetDevice (d);
          uint64_t devImsi = 0;
          Ptr<McUeNetDevice> mcDev = DynamicCast<McUeNetDevice> (dev);
          if (mcDev)
            {
              devImsi = mcDev->GetImsi ();
            }
          else
            {
              Ptr<MmWaveUeNetDevice> mmDev = dev->GetObject<MmWaveUeNetDevice> ();
              if (mmDev)
                devImsi = mmDev->GetImsi ();
            }
          if (devImsi > 0 && devImsi == imsi)
            {
              Ptr<MobilityModel> ueMob = node->GetObject<MobilityModel> ();
              if (ueMob)
                {
                  uePos = ueMob->GetPosition ();
                  ueFound = true;
                }
            }
        }
  ...
  // Compute RSRP from every gNB to this UE — mirrors scenario callback
      ...
      for (uint32_t d = 0; d < node->GetNDevices (); d++)
        {
          ...
          double rsrp = tx_dbm - pl;
          int cid = (int)enbDev->GetCellId ();
          if (cid == (int)m_cellId)
            servingRsrp = rsrp;
          else
            neighRsrpVec.push_back ({cid, rsrp});
        }
    }
}
  ...
  for (auto ue : ueMap)
    {
      ...
      // ── RSRP for serving + all neighbor cells (correct per-cell positions) ──
      double rsrpThisCell;
      std::vector<std::pair<int, double>> rsrpNeighVec;
      ComputeRsrpForAllCells (imsi, rsrpThisCell, rsrpNeighVec);

      // Compute interference-limited SINR: signal / (noise + sum of all neighbor signals)
      // Noise floor: kT=-174 dBm/Hz, BW=20 MHz, NF=5 dB → -96 dBm
      static const double kNoiseFloorDbm = -96.0;
      double sinrServingLimited = -200.0;
      std::vector<std::pair<int, double>> sinrNeighLimited;
      if (rsrpThisCell > -199.0)
        {
          const double noiseLin = std::pow (10.0, kNoiseFloorDbm / 10.0);
          const double srvLin   = std::pow (10.0, rsrpThisCell / 10.0);
          double intrLin = 0.0;
          for (const auto &kv : rsrpNeighVec)
            intrLin += std::pow (10.0, kv.second / 10.0);
          sinrServingLimited = 10.0 * std::log10 (srvLin / (noiseLin + intrLin));

          // Per-neighbor hypothetical SINR (if UE were served by that cell)
          for (const auto &kv : rsrpNeighVec)
            {
              double nLin  = std::pow (10.0, kv.second / 10.0);
              double iLin  = srvLin; // serving becomes interference
              for (const auto &kv2 : rsrpNeighVec)
                if (kv2.first != kv.first)
                  iLin += std::pow (10.0, kv2.second / 10.0);
              sinrNeighLimited.push_back (
                  {kv.first, 10.0 * std::log10 (nLin / (noiseLin + iLin))});
            }
        }

      // Use interference-limited SINR when available; fall back to raw SNR otherwise.
      double sinrForRrc = (sinrServingLimited > -199.0) ? sinrServingLimited : sinrThisCell;

      Ptr<L3RrcMeasurements> l3RrcMeasurementServing;
      if (!indicationMessageHelper->IsOffline ())
        {
          l3RrcMeasurementServing = L3RrcMeasurements::CreateL3RrcUeSpecificSinrServing (
              m_cellId, m_cellId, sinrForRrc);
        }
      NS_LOG_DEBUG (Simulator::Now ().GetSeconds ()
                    << " enbdev " << m_cellId << " UE " << imsi
                    << " L3 serving SINR " << sinrThisCell
                    << " L3 serving RSRP " << rsrpThisCell << " dBm"
      ...
      if (!indicationMessageHelper->IsOffline ())
        {
          l3RrcMeasurementNeigh = L3RrcMeasurements::CreateL3RrcUeSpecificSinrNeigh (m_cellId);
        }
      double sinr;
      std::string neighStr;
      uint16_t bestNeighCellId = 0;
      ...
      // Build lookup: neighbor cellId → interference-limited SINR (dB)
      std::map<int, double> sinrNeighMap;
      for (const auto &kv : sinrNeighLimited)
        sinrNeighMap[kv.first] = kv.second;
      ...
      for (std::map<long double, uint16_t>::iterator it = --sortFlipMap.end ();
           it != --sortFlipMap.begin () && itIndex < nNeighbours; it--)
        {
          ...
          if (it->second != (uint16_t)m_cellId && bestNeighCellId == 0)
            {
              bestNeighCellId = it->second;
              bestNeighSinrDb = sinr;
            }
          convertedSinr = L3RrcMeasurements::ThreeGppMapSinr (sinr);
          if (!indicationMessageHelper->IsOffline ())
            {
              // Use interference-limited SINR if available; fall back to raw SNR.
              int rawCellId = (int)it->second;
              double sinrNeighToAdd = (sinrNeighMap.count (rawCellId) > 0)
                                          ? sinrNeighMap[rawCellId]
                                          : sinr;
              l3RrcMeasurementNeigh->AddNeighbourCellMeasurement (cellId, sinrNeighToAdd);
            }
          NS_LOG_DEBUG (Simulator::Now ().GetSeconds ()
      ...
      // Normalize wideband RSRP → per-RE NR-RSRP (3GPP TS 38.215):
      // SINR computation above already used the wideband values; only the
      // values reported to the xApp via KPM are normalized here.
      static const double kBwNormDb = 10.0 * std::log10 (28.0 * 12.0); // ≈ 25.26 dB
      if (rsrpThisCell > -199.0)
        rsrpThisCell -= kBwNormDb;
      for (auto &kv : rsrpNeighVec)
        kv.second -= kBwNormDb;

      if (!indicationMessageHelper->IsOffline ())
        {
          indicationMessageHelper->AddCuCpUePmItemFull (ueImsiComplete, numDrb, 0,
                                                        l3RrcMeasurementServing,
                                                        l3RrcMeasurementNeigh,
                                                        rsrpThisCell, (int)m_cellId, imsi,
                                                        rsrpNeighVec,
                                                        sinrServingLimited,
                                                        sinrNeighLimited);
        }
...
{  
  ...
  long dlAvailablePrbs = 139; // Correct for numerology 2 (60kHz SCS), TR 38.901
  long ulAvailablePrbs = 139;
  long qci = 1;
  // dlPrbUsage: percentage [0-100] of PRBs used
  long dlPrbUsage = std::min ((long) (prbUtilizationDl / dlAvailablePrbs * 100),
                              (long) 100);
  long ulPrbUsage = 0;

  // ── Also report absolute PRB used (not just %) for xApp Eq.6 ─────────────
  // xApp needs: PRBS = TOTAL_PRB / NUE_serving (paper Eq.14)
  // RRU.PrbUsedDl sends dlPrbUsage (%), xApp can derive: used = dlPrbUsage/100 * 139
  // NUE is already known by xApp from numOfConnectedUEs in serving SINR reports
