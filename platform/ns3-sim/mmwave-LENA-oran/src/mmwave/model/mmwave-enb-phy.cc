...
MmWaveEnbPhy::GetTypeId(void)
{
    static TypeId tid =
        TypeId("ns3::MmWaveEnbPhy")
            ...
            .AddAttribute(
                "NrOverheadFraction",
                "Fraction of TX power always broadcast on all PRBs to model NR overhead signals "
                "(SSB, PDCCH, CSI-RS). Set 0.0 to disable. Default 0.20 (20%).",
                DoubleValue(0.20),
                MakeDoubleAccessor(&MmWaveEnbPhy::m_nrOverheadFraction),
...
MmWaveEnbPhy::CreateTxPowerSpectralDensity()
{
    Ptr<SpectrumValue> psd =
        MmWaveSpectrumValueHelper::CreateTxPowerSpectralDensity(m_phyMacConfig,
                                                                m_txPower,
                                                                m_listOfSubchannels);
    // Add wideband overhead PSD on ALL PRBs to model always-on NR reference signals
    // (SSB, PDCCH, CSI-RS). These occupy all subcarriers in real 5G NR but are absent
    // from the ns-3 data-channel model, leaving close-in UEs artificially noise-limited.
    if (m_nrOverheadFraction > 0.0)
    {
        double overheadPowerDbm = m_txPower + 10.0 * std::log10(m_nrOverheadFraction);
        std::vector<int> allSubchannels;
        for (uint32_t i = 0; i < m_phyMacConfig->GetNumRb(); ++i)
            allSubchannels.push_back(static_cast<int>(i));
        Ptr<SpectrumValue> overheadPsd =
            MmWaveSpectrumValueHelper::CreateTxPowerSpectralDensity(m_phyMacConfig,
                                                                    overheadPowerDbm,
                                                                    allSubchannels);
        (*psd) += (*overheadPsd);
