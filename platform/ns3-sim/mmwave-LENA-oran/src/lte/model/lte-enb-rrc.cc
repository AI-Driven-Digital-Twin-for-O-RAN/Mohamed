...
LteEnbRrc::GetTypeId (void)
{
  ...
  static TypeId tid = TypeId ("ns3::LteEnbRrc")
   ...
   .AddAttribute ("ReportAllUeMeas",
            ...
            MakeBooleanChecker ())
   .AddAttribute ("InitialAssociationDelay",
            "Delay before the first TriggerUeAssociationUpdate call. Set >= Kalman-filter "
            "transient (default 320 ms) to ensure the initial mmWave cell is chosen from "
            "stabilised, filtered SINR measurements rather than a single cold-start sample.",
            TimeValue (MilliSeconds (0)),
            MakeTimeAccessor (&LteEnbRrc::m_initialAssociationDelay),
            MakeTimeChecker ())
    // Trace sources
...
LteEnbRrc::DoRecvUeSinrUpdate(EpcX2SapUser::UeImsiSinrParams params)
{
  ...
  if(!m_ismmWave && !m_interRatHoMode && m_firstReport)
  {
    ...
    // Delay initial cell selection until channel measurements have stabilised.
    // With m_initialAssociationDelay >= Kalman-filter transient (320 ms), the first
    // TriggerUeAssociationUpdate uses filtered, time-averaged SINR rather than the
    // single noisy cold-start sample that fires at t=0 us.
    Simulator::Schedule(m_initialAssociationDelay, &LteEnbRrc::TriggerUeAssociationUpdate, this);
  }
...
LteEnbRrc::PerformE2RCHO (uint64_t imsi, uint16_t targetCellId)
{
  ...
  bool ueInThisCell = (rnti != 0);
  bool alreadyAssociatedImsi = ueInThisCell ||
      (m_mmWaveCellSetupCompleted.find(imsi) != m_mmWaveCellSetupCompleted.end());
  bool onHandoverImsi = true;
  if (alreadyAssociatedImsi)
  {
    if (ueInThisCell)
      onHandoverImsi = false;
    else
      onHandoverImsi = !m_mmWaveCellSetupCompleted.find(imsi)->second;
  }

  NS_LOG_INFO("PerformE2RCHO: imsi " << imsi << " targetCell " << targetCellId
              << " ueInThisCell " << ueInThisCell << " alreadyAssociated " << alreadyAssociatedImsi
              << " onHandover " << onHandoverImsi);
  ...
  if (rnti == 0)
  {
    NS_LOG_WARN("## Warn: handover not triggered - RNTI is 0 for IMSI " << imsi);
    return;
  }

  NS_LOG_UNCOND(Simulator::Now().GetNanoSeconds() / 1.0e9
                << "s LteEnbRrc::PerformE2RCHO(): Initiate handover for rnti "
  ...
  if(m_imsiCellSinrMap.size() > 0) // there are some entries
  {
    for(std::map<uint64_t, CellSinrMap>::iterator imsiIter = m_imsiCellSinrMap.begin(); imsiIter != m_imsiCellSinrMap.end(); ++imsiIter)
    {
      ...
      else
      {
        ...
        // Deferred initial mmWave access: fires when TriggerUeAssociationUpdate was
        // delayed past the UE's CONNECTED_NORMALLY transition. In that case the
        // m_pendingConnectToMmWave flag was cleared with no best-cell entry, so
        // SendRrcConnectToMmWave was never called.  Detect the condition by:
        //   – mmWave setup started but never completed (m_mmWaveCellSetupCompleted = false)
        //   – no mmWave cell was ever assigned (m_lastMmWaveCell has no entry for this IMSI)
        bool deferredInitialConnect = false;
        {
          auto mmWaveSetupIt  = m_mmWaveCellSetupCompleted.find(imsi);
          // Line 4354 uses operator[] on m_lastMmWaveCell, creating a default 0 entry
          // before we get here.  A real mmWave cell ID is always > 0, so value == 0
          // reliably means the UE never connected to any mmWave cell.
          bool neverHadMmWave = (m_lastMmWaveCell[imsi] == 0);
          uint16_t rnti       = GetRntiFromImsi(imsi);
          if (rnti != 0 &&
              mmWaveSetupIt != m_mmWaveCellSetupCompleted.end() && !mmWaveSetupIt->second &&
              neverHadMmWave &&
              maxSinrDb > m_outageThreshold)
          {
            NS_LOG_INFO("Deferred initial mmWave access for IMSI " << imsi
                        << " -> cell " << maxSinrCellId
                        << " SINR=" << maxSinrDb << " dB");
            m_rrcSapUser->SendRrcConnectToMmWave(rnti, maxSinrCellId);
            deferredInitialConnect = true;
          }
        }

        if (!deferredInitialConnect)
        {
