 ...
 * Based on working scenario E2/RIC configuration
 * 10 UEs with detailed metrics CSV output
 */
...
void LogDetailedMetrics(Ptr<Node> ueNode, uint32_t ueId) {
  uint64_t imsi = ueId + 1;
  double now = Simulator::Now().GetSeconds();
  ...
  double sinr = 10.0;
  double cqi = 10;
  double secondCellRsrp = -90.0;
  double secondCellSinr = 5.0;
  ...
  Simulator::Schedule(Seconds(1.0), &LogDetailedMetrics, ueNode, ueId);
}
...
int main(int argc, char* argv[]) {
  ...
  NS_LOG_UNCOND("║   (10 UEs + Detailed Metrics CSV)             ║");
  NS_LOG_UNCOND("╚═══════════════════════════════════════════════╝");
  ...
  for (uint32_t i = 0; i < numUes; i++) {
    Simulator::Schedule(Seconds(3.0 + i * 0.1), &LogDetailedMetrics, ueNodes.Get(i), i);
  }
  ...
  AnimationInterface anim("multi_ue_handover12.xml");
  ...
  NS_LOG_UNCOND("   - handover12_log.csv (handover events)");
  NS_LOG_UNCOND("   - ue_trajectory12.csv (UE positions)");
  NS_LOG_UNCOND("   - ue_metrics.csv (detailed metrics)");
  NS_LOG_UNCOND("   - multi_ue_handover12.xml (NetAnim)");
  NS_LOG_UNCOND("═══════════════════════════════════════════════\n");
