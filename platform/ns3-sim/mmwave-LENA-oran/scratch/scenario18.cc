 ...
 * - Automatic Secondary Cell Change on Inter-eNB Handover
 * - LTE Cell 1→2: mmWave 4→5
 * - LTE Cell 2→3: mmWave 5→6
 * - LTE Cell 3→1: mmWave 6→4
 */
...
void TriggerSecondaryCellChange(uint64_t imsi, uint16_t newLteCell) {
    ...
    if (ueIndex >= globalMcUeDevs.GetN()) {
        NS_LOG_ERROR("⚠️ Invalid UE index: " << ueIndex);
        return;
    }
    
    Ptr<NetDevice> ueDevice = globalMcUeDevs.Get(ueIndex);
    Ptr<McUeNetDevice> mcUeDev = DynamicCast<McUeNetDevice>(ueDevice);
    
    if (!mcUeDev) {
        NS_LOG_ERROR("⚠️ Could not cast to McUeNetDevice");
        return;
    }
    
    // Find target mmWave eNB
    Ptr<NetDevice> targetMmWaveEnb = nullptr;
    for (uint32_t i = 0; i < globalMmWaveEnbDevs.GetN(); i++) {
        Ptr<MmWaveEnbNetDevice> enbDev = 
            DynamicCast<MmWaveEnbNetDevice>(globalMmWaveEnbDevs.Get(i));
        if (enbDev && enbDev->GetCellId() == newMmWaveCell) {
            targetMmWaveEnb = globalMmWaveEnbDevs.Get(i);
            break;
        }
    }
    
    if (!targetMmWaveEnb) {
        NS_LOG_ERROR("⚠️ Target mmWave eNB not found for Cell " << newMmWaveCell);
        return;
    }
    
    NS_LOG_UNCOND("🔧 Performing Secondary Cell Change...");
...
void NotifyHandoverEnd(std::string context, uint64_t imsi, 
  ...
  NS_LOG_UNCOND("⏱️  Time: " << now << "s");
  NS_LOG_UNCOND("📡 Old LTE Cell: " << oldLteCell << " → New LTE Cell: " << cellId);
  ...
  if (oldLteCell != cellId && oldLteCell != 0) {
      NS_LOG_UNCOND("🔄 Inter-eNB HO detected! Triggering Secondary Cell Change...");
      Simulator::Schedule(MilliSeconds(100), &TriggerSecondaryCellChange, imsi, cellId);
  } else {
      // Same cell or initial connection - just update GUI
      if (imsi >= 1 && imsi <= 20) {
...
int main(int argc, char* argv[]) {
  ...
  NS_LOG_UNCOND("║   ✅ WITH SECONDARY CELL CHANGE               ║");
  NS_LOG_UNCOND("║   🔄 LTE HO triggers mmWave Secondary Change  ║");
  NS_LOG_UNCOND("╚═══════════════════════════════════════════════╝");
