...
// ============================================================
//  SINR Moving Average — IMPROVEMENT #1
// ============================================================
/*
 * [IMPROVEMENT #1 — SINR Moving Average بدل Immediate Decision]
 * PURPOSE : منع الـ xApp من اتخاذ قرار HO بناءً على قراءة SINR واحدة متذبذبة (noisy sample).
 *
 * WHAT IT DOES :
 *   - بدل ما نستخدم قيمة SINR المباشرة من آخر KPM report،
 *     بنحتفظ بـ circular buffer من آخر SINR_AVG_WINDOW قراءات.
 *   - sinr_avg = متوسط آخر SINR_AVG_WINDOW قراءات.
 *   - getTargetCellID() بيستخدم sinr_avg للمقارنة بدل sinr المباشر.
 *   - لو البـ buffer لسه مش مملي (أول 3 KPM cycles)، بيرجع للقيمة المباشرة fallback.
 *
 * WHY : في بيئة mmWave، الـ SINR بيتذبذب بشدة. قراءة واحدة منخفضة ممكن تسبب
 *       HO غير ضروري، والقراءة التالية ترجع عادية → ping-pong.
 *       المتوسط بيخلي القرار أكثر استقراراً وأقل حساسية للـ noise.
 *
 * SINR_AVG_WINDOW = 3 : قيمة متوازنة — استجابة سريعة + تنعيم كافٍ.
 *   زيادتها → استجابة أبطأ لكن أكثر استقراراً.
 *   تقليلها → استجابة أسرع لكن أقل تنعيماً.
 */
#define SINR_AVG_WINDOW 3

struct SINRServingValues
{
  bool is_available;
  uint16_t ueID;
  double sinr;                           // latest raw reading
  double sinr_avg;                       // smoothed value → used for HO decisions
  double sinr_buf[SINR_AVG_WINDOW];      // circular buffer
  int    sinr_buf_idx;                   // next write slot
  int    sinr_buf_count;                 // samples stored (≤ SINR_AVG_WINDOW)
  struct SINRNeighboringValues* neighCells;
...
/*
 * [IMPROVEMENT #1 — update_sinr_avg]
 * PURPOSE : تحديث الـ circular buffer وإرجاع المتوسط الجديد.
 * WHAT IT DOES :
 *   1. بيكتب الـ sample الجديد في المكان الصح في الـ buffer (circular).
 *   2. بيحرك الـ write pointer للخانة الجاية.
 *   3. بيزود العداد لحد ما يوصل SINR_AVG_WINDOW.
 *   4. بيحسب المتوسط على عدد الـ samples الفعلية (مش ثابت 3 من الأول).
 * WHY : استخدمنا circular buffer بدل array عادي عشان نتجنب memory realloc
 *       في كل KPM cycle — الأداء أفضل مع الـ high-frequency callbacks.
 */
static double update_sinr_avg(struct SINRServingValues* ue, double new_sinr)
{
    ue->sinr_buf[ue->sinr_buf_idx] = new_sinr;
    ue->sinr_buf_idx = (ue->sinr_buf_idx + 1) % SINR_AVG_WINDOW;
    if (ue->sinr_buf_count < SINR_AVG_WINDOW) ue->sinr_buf_count++;

    double sum = 0.0;
    for (int k = 0; k < ue->sinr_buf_count; k++) sum += ue->sinr_buf[k];
    return sum / ue->sinr_buf_count;
}
...
void add_UE(struct SINR_Map* cell, const uint16_t ueID, const double sinr) 
{
  ...
  /*
   * [IMPROVEMENT #1 — apply moving average on each new SINR reading]
   * sinr     : بيتحدث بالقيمة الخام من الـ KPM report (محتاجة للـ logging).
   * sinr_avg : بيتحسب عن طريق update_sinr_avg → ده اللي بيتستخدم في قرار الـ HO.
   *
   * [IMPROVEMENT #4 — log SINR snapshot to sinr_xapp.csv]
   * كل مرة بيجي KPM reading لـ UE، بنسجل (time, ue_id, cell_id, sinr) في الـ CSV.
   * ده بيعملنا time-series لرسم SINR vs Time لكل UE في الـ results.
   */
  cell->connectedUEs[ueID].ueID     = ueID;
  // Update SINR + compute moving average
  cell->connectedUEs[ueID].sinr     = sinr;
  cell->connectedUEs[ueID].sinr_avg = update_sinr_avg(&cell->connectedUEs[ueID], sinr);
...
uint16_t getTargetCellID(callback_data_t data)
{
    ...
    /*
     * [IMPROVEMENT #1 — Use averaged SINR for HO decision instead of raw SINR]
     * PURPOSE : استخدام المتوسط بدل القيمة اللحظية في مقارنة serving vs neighbor SINR.
     * WHAT IT DOES :
     *   - لو الـ buffer اتملى (sinr_buf_count >= SINR_AVG_WINDOW) → استخدم sinr_avg.
     *   - لو لسه في الأول (أقل من 3 cycles) → fallback للقيمة المباشرة sinr.
     *     ده عشان مانبطلش القرار خالص في أول ثوانٍ من تشغيل الـ xApp.
     * WHY : قرار الـ HO بيتوقف على (serving_sinr vs neighbor_sinr + threshold).
     *       لو serving_sinr قيمة واحدة منخفضة بسبب fading → HO غلط.
     *       المتوسط بيحمي من ده.
     */
    // Use smoothed average SINR — IMPROVEMENT #1 (avoids triggering on a single noisy sample)
    double serving_sinr = (serving_ue->sinr_buf_count >= SINR_AVG_WINDOW)
                           ? serving_ue->sinr_avg
                           : serving_ue->sinr;   // fall back to raw until buffer is full
    ...
    for (int i = 0; i < MAX_REGISTERED_NEIGHBOURS; i++)
    {
        ...
        if (neighbor_sinr > (serving_sinr + HANDOVER_THRESHOLD_DB)) {
            /*
             * [IMPROVEMENT #3 — Hysteresis Layer-2 Check inside getTargetCellID]
             * PURPOSE : رفض الـ candidate cell لو الـ UE رجعلها قريب قوي حتى لو
             *           الـ SINR فارقه أكبر من HANDOVER_THRESHOLD_DB.
             * WHAT IT DOES :
             *   - بيشيك لو g_ue_prev_cell[ue_id] == neighbor cell (الـ UE سابها قبل كده).
             *   - لو فارق الوقت أقل من PING_PONG_WINDOW_SEC → required_gain تتزود بـ HYSTERESIS_DB.
             *   - يعني الفارق المطلوب بقى (3 + 2 = 5 dB) بدل 3 dB عادي.
             *   - لو الـ neighbor مش بيوفي الـ required_gain المعززة → يتعمل skip وتظهر رسالة.
             * WHY : الـ time guard (MIN_HO_INTERVAL_SEC) في evaluateHandoverOpportunities
             *       بيحمي لفترة 5 ثواني. لكن بعد 5 ثواني لو الـ SINR اتذبذب بـ 3.1 dB
             *       هيعمل HO تاني. الـ hysteresis بيطلب 5 dB → يضمن إن التحسن حقيقي.
             */
            // Layer-2 Hysteresis: if we recently left this neighbour cell,
            // require an extra HYSTERESIS_DB gain before returning to it.
            double required_gain = HANDOVER_THRESHOLD_DB;
            ...
            if (ue_id_chk > 0 && ue_id_chk < MAX_REGISTERED_UES &&
                ...
                if (since < PING_PONG_WINDOW_SEC) {
                    required_gain = HANDOVER_THRESHOLD_DB + HYSTERESIS_DB;
                    LOG_INFO("UE %d  hysteresis applied for Cell %d  required gain %.1fdB",
                             ue_id_chk, i, required_gain);
                }
            }
            if (neighbor_sinr <= (serving_sinr + required_gain)) {
                printf(CLR_BMAGENTA "  │" CLR_RESET "  Cell " CLR_DIM "%2d" CLR_RESET
                       "  SINR " CLR_DIM "%+7.2f dB" CLR_RESET
                       "  " CLR_DIM "  blocked by hysteresis" CLR_RESET "\n",
                       i, neighbor_sinr);
                continue;
            }
...
int main(int argc, char* argv[]) 
{
  ...
  printf("\n" CLR_BBLUE
    ...
    "  ║  " CLR_CYAN "Decision Mode      " CLR_BWHITE "SINR moving average (window=3)" CLR_BBLUE "     ║\n"
    ...
    if (global_ho_in_progress()) {
        ...
        for (int _ci = 0; _ci < MAX_REGISTERED_CELLS && next_sleep_us > ADAPTIVE_SLEEP_SHORT_US; _ci++) {
            ...
            for (int _ui = 1; _ui < MAX_REGISTERED_UES; _ui++) {
                ...
                double _s = (_c->connectedUEs[_ui].sinr_buf_count > 0)
                             ? _c->connectedUEs[_ui].sinr_avg
                             : _c->connectedUEs[_ui].sinr;
