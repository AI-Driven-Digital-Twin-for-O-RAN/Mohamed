 ...
 */
#define ADAPTIVE_SLEEP_EMERGENCY_US 1000000  //  1.0 s — critical cell overload (≥5 UEs)
...
static const double __attribute__((unused)) g_ue_speed_mps[MAX_REGISTERED_UES] = {
    ...
    /* 7  */ 18.0,  /* UE7  vehicle — Cell 2, crosses boundaries after offload */
    /* 8  */ 22.0,  /* UE8  vehicle — Cell 2, crosses boundaries after offload */
 ...
 */
// Ping-pong: UE returning to any of the last CELL_HISTORY_LEN cells within window.
#define PING_PONG_WINDOW_SEC 9999
#define CELL_HISTORY_LEN     4
typedef struct { uint16_t cell; time_t left_at; } CellVisit_t;
static CellVisit_t g_ue_cell_hist[MAX_REGISTERED_UES][CELL_HISTORY_LEN];
static int         g_ue_cell_hist_idx[MAX_REGISTERED_UES];
static int         g_total_ping_pong_count = 0;

static void cell_hist_push(uint16_t ue_id, uint16_t cell)
{
    if (ue_id == 0 || ue_id >= MAX_REGISTERED_UES) return;
    int idx = g_ue_cell_hist_idx[ue_id];
    g_ue_cell_hist[ue_id][idx].cell    = cell;
    g_ue_cell_hist[ue_id][idx].left_at = time(NULL);
    g_ue_cell_hist_idx[ue_id] = (idx + 1) % CELL_HISTORY_LEN;
}

static int cell_hist_recently_visited(uint16_t ue_id, uint16_t cell, double window_sec)
{
    if (ue_id == 0 || ue_id >= MAX_REGISTERED_UES) return 0;
    time_t now = time(NULL);
    for (int i = 0; i < CELL_HISTORY_LEN; i++) {
        CellVisit_t* v = &g_ue_cell_hist[ue_id][i];
        if (v->cell == cell && v->left_at > 0 && difftime(now, v->left_at) < window_sec)
            return 1;
    }
    return 0;
}
...
// Detect ping-pong: returns 1 if this HO revisits any of the last CELL_HISTORY_LEN cells.
static int metrics_is_ping_pong(uint16_t ue_id, uint16_t from_cell, uint16_t to_cell)
{
    if (ue_id == 0 || ue_id >= MAX_REGISTERED_UES) return 0;
    int is_pp = cell_hist_recently_visited(ue_id, to_cell, PING_PONG_WINDOW_SEC);
    if (is_pp) {
        g_total_ping_pong_count++;
        LOG_WARN("Ping-pong detected  UE " CLR_BRED "%d" CLR_RESET
                 "  %d→%d  (revisit within history window)",
                 ue_id, from_cell, to_cell);
    }
    cell_hist_push(ue_id, from_cell);
    return is_pp;
}
...
if (serving_ue->prb_used > 0) {
    ...
    for (int i = 0; i < MAX_REGISTERED_NEIGHBOURS; i++) {
        ...
        /* Block cells the UE recently visited — prevents A→B→C→A oscillation.
         * Windows calibrated to sim/real ratio ~0.0096: vehicle 900s≈8.6 sim-sec,
         * cyclist 1800s≈17 sim-sec, pedestrian 3600s≈35 sim-sec. */
        {
            double spd_i = (data.ueID > 0 && data.ueID < MAX_REGISTERED_UES)
                            ? g_ue_speed_mps[data.ueID] : 5.0;
            double pp_win = (spd_i <= 2.0) ? 3600.0 : (spd_i <= 8.0) ? 1800.0 : 900.0;
            if (cell_hist_recently_visited(data.ueID, (uint16_t)i, pp_win)) {
                printf("  │  Cell %2d: UE%d visited recently (%.0fs window) → " CLR_BYELLOW "HISTORY BLOCKED\n" CLR_RESET,
                       i, data.ueID, pp_win);
                continue;
            }
        }
    ...
    /* Anti-bounce hard block: if best NCL target was recently visited, block return. */
    uint16_t ue_id_chk = (uint16_t)data.ueID;
    if (ue_id_chk > 0 && ue_id_chk < MAX_REGISTERED_UES &&
        cell_hist_recently_visited(ue_id_chk, (uint16_t)ncl[best_ncl_idx].cell_id, PING_PONG_WINDOW_SEC)) {
        {
        double since = 0.0; /* already caught by NCL history gate above */
        if (1) {
            printf("  │  " CLR_BRED "Anti-bounce: Cell %d in visit history → HO blocked\n"
                   CLR_RESET, ncl[best_ncl_idx].cell_id);
            printf(CLR_BBLUE
                   "  ╚═════════════════════════════════════════════════════════════════╝\n"
                   CLR_RESET "\n");
            return 0;
...
void evaluateHandoverOpportunities(Callback targetCellFinding, Callback cbHOAction, callback_data_t data) 
{
  ...
  for (int i = 0; i < MAX_REGISTERED_CELLS; i++) 
  {
    ...
    for (int j = 0; j < MAX_REGISTERED_UES; j++) 
    {
       ...
       * Calibrated to sim/real ratio ~0.0096: vehicle 900s≈8.6 sim-sec,
       * cyclist 1800s≈17 sim-sec, pedestrian 3600s≈35 sim-sec.
       * EMERGENCY OVERRIDE: if serving cell has ≥5 UEs (critical overload),
       * bypass guard entirely so the xApp re-balances immediately. */
      int cell_critical = ((int)cell->numOfConnectedUEs >= 5);
      ...
      if (ctx->last_handover_time > 0) {
        ...
        double adaptive_guard = (spd_mps <= 2.0) ? 3600.0 :  /* pedestrian  */
                                (spd_mps <= 8.0) ? 1800.0 :  /* walker/cyclist */
                                                    900.0;    /* vehicle     */
        if (time_since_last_ho < adaptive_guard && !cell_critical) {
        ...
        if (cell_critical && time_since_last_ho < adaptive_guard) {
          LOG_WARN("EMERGENCY override: Cell %d  %zu UEs — guard bypassed (%.0fs elapsed)",
                   cell->cellID, cell->numOfConnectedUEs, time_since_last_ho);
        }
      }
        ...
        if (lb_target != 0) {
          ...
          if (cbHOAction(ue_data)) {
            ...
            LOG_OK("[LB#4] Load-Balancing HO sent  UE " CLR_BWHITE "%d" CLR_RESET
                   ...
                   ue->ueID, cell->cellID, lb_target);
            /* Record LB departure so history gate blocks the bounce-back. */
            cell_hist_push(ue->ueID, (uint16_t)cell->cellID);
            return;
          } else {
  ...
  while (1) {
    ...
    if (global_ho_in_progress()) {
        ...
        for (int _ci = 0; _ci < MAX_REGISTERED_CELLS; _ci++) {
            ...
            // Emergency: cell critically overloaded → react in 1s
            if ((int)_c->numOfConnectedUEs >= 5) {
                next_sleep_us = ADAPTIVE_SLEEP_EMERGENCY_US;
                break;
            }
            // Scan UEs for poor SINR
            for (int _ui = 1; _ui < MAX_REGISTERED_UES && next_sleep_us > ADAPTIVE_SLEEP_SHORT_US; _ui++) {
