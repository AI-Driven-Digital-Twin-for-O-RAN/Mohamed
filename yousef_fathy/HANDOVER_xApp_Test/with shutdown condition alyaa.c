...
// ctrl after this
const int MAX_NUM_OF_RIC_INDICATIONS = 3; // 3 * 40ms = 120ms
...
handover_context_t ho_context; // استبدال bool بـ handover_context_t
};
...
uint8_t getTargetCellID(callback_data_t data)
{
    ...
    // A3 Event parameters (matching NS-3 config)
    const double A3_OFFSET = 2.0;      // dB
    const double HYSTERESIS = 0.5;     // dB
    const double A3_THRESHOLD = serving_sinr + A3_OFFSET + HYSTERESIS;
    
    printf("\n=== A3 Event Evaluation for UE %d ===\n", data.ueID);
    printf("Serving Cell %d: SINR = %.2f dB\n", data.frmCurntCell, serving_sinr);
    printf("A3 Threshold = %.2f dB (serving + %.1f + %.1f)\n",
           A3_THRESHOLD, A3_OFFSET, HYSTERESIS);
    ...
    for (int i = 0; i < MAX_REGISTERED_NEIGHBOURS; i++)
    {
        ...
        if (data.neighCells[i].counter < MAX_NUM_OF_RIC_INDICATIONS) {
            printf("  Cell %d: SINR %.2f dB - insufficient samples (%d/%d)\n",
        ...
        if (cells_sinr_map[i].sinrMap != NULL && 
            ...
            printf("  Cell %d: SKIPPED (marked for shutdown)\n", i);
            continue;
        }
        
        double neighbor_sinr = data.neighCells[i].sinr;
        
        // A3 Event: Neighbor > Serving + Offset + Hysteresis
        if (neighbor_sinr > A3_THRESHOLD) {
            double gain = neighbor_sinr - serving_sinr;
            printf("  Cell %d: SINR %.2f dB - A3 TRIGGERED (gain: %.2f dB)\n",
            ...
            printf("  Cell %d: SINR %.2f dB - below A3 threshold\n",
                   i, neighbor_sinr);
        }
    }
    ...
    if (targetCell != 0 && targetCell != data.frmCurntCell) {
        ...
        printf("  ✗ No suitable target cell found\n");
    }
...
{
//alyaa  assert(msg_frm_1->meas_info_lst_len > 0 && "Cannot correctly print measurements");

// Validation for NS-3 KPM data

// Enhanced validation for NS-3 KPM data
if (msg_frm_1 == NULL ||
    msg_frm_1->meas_info_lst_len == 0 || 
    msg_frm_1->meas_info_lst_len > 1000 ||
    msg_frm_1->meas_data_lst_len == 0 ||
    msg_frm_1->meas_data_lst_len > 1000000) {  // ← السطر ده مهم!
...
{
// fprintf(stderr, "[TEMP] KPM callback disabled\n");
 //  return;
  assert(rd != NULL);
  ...
  
  {
    ...
    // ============================================
    // ADD THESE CHECKS ONLY - DON'T CHANGE ANYTHING ELSE
    // ============================================
    if (msg_frm_3 == NULL || 
    ...
    // KEEP EVERYTHING BELOW AS IS - DON'T CHANGE
        ...
        !processed_cells[i]) 
    {
      ...
      for (int j = 0; j < MAX_REGISTERED_UES; j++) {
        if (cell->connectedUEs[j].is_available && 
            ...
            cell->connectedUEs[j].ho_context.state == HO_STATE_IDLE)	
 {
...
uint16_t doHandoverAction(callback_data_t data)
{
    ...
    if (ue->ho_context.state == HO_STATE_IN_PROGRESS) {
        ...
        if (elapsed < MAX_HANDOVER_TIMEOUT_SEC) {
            ...
            printf("[xApp] Handover timeout for UE %d (%.1fs), retrying...\n",
                   data.ueID, elapsed);
    ...
    if (ue->ho_context.attempts >= MAX_HANDOVER_ATTEMPTS) {
        printf("[xApp] Max handover attempts (%d) reached for UE %d\n",
               MAX_HANDOVER_ATTEMPTS, data.ueID);
        return 0;
    }
