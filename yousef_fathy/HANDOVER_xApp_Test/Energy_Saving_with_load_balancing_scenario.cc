...
ns3::DoubleValue(0.04), ns3::MakeDoubleChecker<double>(0.01, 2.0));
...
void OnHandoverStart(std::string context, uint64_t imsi, uint16_t sourceCellId, uint16_t rnti, uint16_t targetCellId) 
{
    ...
    if (g_handover_log.is_open()) {
        g_handover_log << time << ",START," << imsi << "," 
                      << sourceCellId << "," << targetCellId << std::endl;
    }
}
...
void OnHandoverSuccess(std::string context, uint64_t imsi, uint16_t cellId, uint16_t rnti) 
{
    ...
    // Log to file
    if (g_handover_log.is_open()) {
        g_handover_log << time << ",SUCCESS," << imsi << "," 
                      << cellId << "," << (expected ? "1" : "0") << std::endl;
    }
}
...
void OnHandoverFailure(std::string context, uint64_t imsi, uint16_t sourceCellId) 
{
    ...
    // Log to file
    if (g_handover_log.is_open()) {
        g_handover_log << time << ",FAILURE," << imsi << "," 
                      << sourceCellId << ",0" << std::endl;
    }
}
...
// ============================================
 ...
 // Carrier bandwidth in Hz
  ...
  ueNodes.Create(nUeNodes);  // 10 UEs فقط
  ...
  // ============================================
  // MODIFIED: UE Mobility - Fast and Very Slow Moving UEs
  // ============================================
  ...
  // أول 6 UEs: متحركة بسرعة عادية (مقسمة لمجموعتين)
  uint32_t nFastUEs = 6;
  uint32_t halfFastUEs = nFastUEs / 2;
  
  // المجموعة الأولى من السريعة: تبدأ من X = 2500
  for (uint32_t i = 0; i < halfFastUEs; ++i) {
      double startX = 2300;
      double startY = randY->GetValue();
      uePositionAlloc->Add(Vector(startX, startY, 1.5));
  }
  
  // المجموعة الثانية من السريعة: تبدأ من X = 2000
  for (uint32_t i = halfFastUEs; i < nFastUEs; ++i) {
      double startX = 2000;
      double startY = randY->GetValue();
      uePositionAlloc->Add(Vector(startX, startY, 1.5));
  }
  
  // آخر 4 UEs: بطيئة جداً بإحداثيات محددة
  uePositionAlloc->Add(Vector(2500, 1900, 1.5));  // UE 7
  uePositionAlloc->Add(Vector(2000, 1900, 1.5));  // UE 8
  uePositionAlloc->Add(Vector(1750, 1400, 1.5));  // UE 9
  uePositionAlloc->Add(Vector(1750, 2250, 1.5));  // UE 10
  ...
  // تعيين السرعة للـ UEs
  Ptr <UniformRandomVariable> speedRand = CreateObject<UniformRandomVariable>();
  speedRand->SetAttribute("Min", DoubleValue(150.0));
  speedRand->SetAttribute("Max", DoubleValue(200.0));
  
  // أول 6 UEs: سرعة عادية (10-20 m/s)
  for (uint32_t i = 0; i < nFastUEs; ++i) {
      ...
      mob->SetVelocity(Vector(-speed, 0.0, 0.0)); // حركة من اليمين لليسار
  }
  
  // آخر 4 UEs: سرعة بطيئة جداً جداً (0.1-0.5 m/s)
  Ptr <UniformRandomVariable> slowSpeedRand = CreateObject<UniformRandomVariable>();
  slowSpeedRand->SetAttribute("Min", DoubleValue(220));
  slowSpeedRand->SetAttribute("Max", DoubleValue(280));
  
  for (uint32_t i = nFastUEs; i < nUeNodes; ++i) {
      Ptr<ConstantVelocityMobilityModel> mob = ueNodes.Get(i)->GetObject<ConstantVelocityMobilityModel>();
      double speed = slowSpeedRand->GetValue();
      mob->SetVelocity(Vector( speed, 0.0, 0.0)); // حركة بطيئة جداً من اليمين لليسار
  }
  
  NS_LOG_UNCOND("Created " << nFastUEs << " fast-moving UEs (10-20 m/s) and " 
                << (nUeNodes - nFastUEs) << " very slow-moving UEs (0.1-0.5 m/s)");
  // ============================================
...
// ============================================
  ...
  // ============================================
  g_handover_log.open("handover_log.csv", std::ios::out | std::ios::trunc);
  if (g_handover_log.is_open()) {
      g_handover_log << "Time,Event,IMSI,CellID,Success" << std::endl;
      NS_LOG_UNCOND("Handover log file created: handover_log.csv");
  }
