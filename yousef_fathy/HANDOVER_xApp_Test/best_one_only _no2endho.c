...
void add_UE(struct SINR_Map* cell, const uint16_t ueID, const double sinr) 
{
  ...
  // HANDOVER RACE CONDITION FIX: Check if this UE recently completed handover FROM this cell
  // If so, ignore stale KPM measurements that still report it here
  if (is_new_ue) {
    // Check all other cells to see if UE is there with recent handover
    for (int other_cell_idx = 0; other_cell_idx < MAX_REGISTERED_CELLS; other_cell_idx++) {
      if (other_cell_idx == cell->cellID || !cells_sinr_map[other_cell_idx].is_registered)
        continue;
      
      struct SINR_Map* other_cell = cells_sinr_map[other_cell_idx].sinrMap;
      if (other_cell != NULL && other_cell->connectedUEs != NULL &&
          other_cell->connectedUEs[ueID].is_available) {
        
        // UE exists in another cell - check if it recently completed HO
        time_t now = time(NULL);
        double time_since_completion = difftime(now, other_cell->connectedUEs[ueID].ho_context.completion_time);
        
        if (time_since_completion < 2.0) {  // Within 2 seconds of completion
          // This is likely a stale measurement - UE has already moved
          printf("[UE Tracking] Ignoring stale measurement: UE %d in cell %d (already in cell %d)\n",
                 ueID, cell->cellID, other_cell->cellID);
          return;  // Don't add UE to this cell
        }
      }
    }
  }
  ...
  if (is_new_ue) {
    ...
    cell->connectedUEs[ueID].ho_context.completion_time = 0;
