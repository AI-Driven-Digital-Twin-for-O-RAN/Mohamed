...
// SINR-Based Handover xApp - FIXED VERSION
// Pure SINR-driven mobility management with proper state tracking
//
// KEY FIXES:
...
#include "lstm_predictor_client.h"  // GRU/LSTM handover prediction service
...
static bool g_gru_available = false;  // GRU prediction service availability
    ...
    for (int cell_idx = 0; cell_idx < MAX_REGISTERED_CELLS; cell_idx++) {
        ...
        for (int ue_idx = 0; ue_idx < MAX_REGISTERED_UES; ue_idx++) {
            ...
            if (a3_result.a3_triggered) {
                ...
                // ============================================
                // GRU MODEL CONSULTATION (Ping-Pong Avoidance)
                // ============================================
                bool execute_ho = true;
                uint16_t final_target = a3_result.best_target_cell;
                lstm_prediction_result_t gru_result = {0};

                if (g_gru_available) {
                    lstm_ue_measurements_t meas = {
                        .ue_id            = ue->ueID,
                    ...
                    printf("[GRU] Querying prediction service for UE %d...\n", ue->ueID);

                    pthread_mutex_unlock(&mtx);
                    int pred_rc = lstm_predict_handover(&meas, &gru_result);
                    pthread_mutex_lock(&mtx);
                    if (pred_rc == 0 && gru_result.prediction_success) {

                        printf("[GRU] Decision: %s | Confidence: %.2f | ToS: %.2fs\n",
                               gru_result.should_handover ? "EXECUTE" : "AVOID",
                               gru_result.confidence,
                               gru_result.time_to_handover);

                        if (!gru_result.should_handover) {
                            ...
                            execute_ho = false;
                            printf("[GRU] BLOCKED: Handover to Cell %d suppressed "
                                   "(ToS=%.2fs, confidence=%.2f)\n",
                                   a3_result.best_target_cell,
                                   gru_result.time_to_handover,
                                   gru_result.confidence);
                        } else if (gru_result.should_handover &&
                                   gru_result.target_cell != 0 &&
                                   gru_result.target_cell < MAX_REGISTERED_CELLS &&
                                   gru_result.target_cell != a3_result.best_target_cell &&
                                   cells_sinr_map[gru_result.target_cell].is_registered) {
                            // GRU recommends a different (better) target cell
                            printf("[GRU] Redirecting: SINR pick=Cell %d -> GRU pick=Cell %d\n",
                                   a3_result.best_target_cell, gru_result.target_cell);
                            final_target = gru_result.target_cell;
                        }
                    } else {
                        printf("[GRU] Prediction failed -- falling back to SINR decision\n");
                    }
                }
                ...
                if (g_decision_log) {
                    fprintf(g_decision_log, "%lu,%ld,%d,%d,%.2f,%.2f,%d,%.2f,%s,%.3f,%.3f,0,0,%s\n",
                        ...
                        gru_result.prediction_success ? (gru_result.should_handover ? "EXECUTE" : "AVOID") : "NO_PRED",
                        gru_result.confidence, gru_result.time_to_handover,
...
int main(int argc, char* argv[]) {
    ...
    printf("╔══════════════════════════════════════════════════════════════╗\n");
    printf("║                                                              ║\n");
    printf("║       GRU-ENHANCED HANDOVER xAPP - INTEGRATED VERSION       ║\n");
    printf("║       SINR A3 + GRU Ping-Pong Avoidance (Fares Model)       ║\n");
    printf("║                                                              ║\n");
    printf("╚══════════════════════════════════════════════════════════════╝\n\n");
    ...
    // Initialize GRU prediction service
    printf("[Init] Connecting to GRU prediction service (Fares model)...\n");
    if (lstm_predictor_init() == 0 && lstm_service_available()) {
        g_gru_available = true;
        printf("[Init] ✅ GRU prediction service connected — ping-pong avoidance ACTIVE\n");
    } else {
        printf("[Init] ⚠️  GRU service unavailable — running in SINR-only mode\n");
        printf("[Init]     Start service: python3 HANDOVER_xApp_Test/lstm_predictor_service.py\n");
    }
    ...
    printf("  - GRU Model: %s\n", g_gru_available ? "ACTIVE (ping-pong avoidance ON)" : "OFFLINE (SINR-only fallback)");
    printf("  - GRU Confidence Threshold: 0.60\n\n");
    ...
    // Cleanup GRU prediction service
    if (g_gru_available) {
        lstm_predictor_cleanup();
    }
    ...
    printf("\n╔══════════════════════════════════════════════════════╗\n");
    printf("║              GRU SESSION SUMMARY                    ║\n");
    printf("╠══════════════════════════════════════════════════════╣\n");
    printf("║  Total A3 triggers    : %5lu                        ║\n", g_total_a3_triggers);
    printf("║  Handovers EXECUTED   : %5lu                        ║\n", g_total_executed);
    printf("║  Handovers BLOCKED    : %5lu (GRU avoidance)        ║\n", g_total_gru_blocked);
