...
EpcX2::DoSendHandoverRequest(EpcX2SapProvider::HandoverRequestParams params)
{
    ...
    if (m_x2InterfaceSockets.find(params.targetCellId) == m_x2InterfaceSockets.end())
    {
        NS_LOG_WARN("[EpcX2] DoSendHandoverRequest: no X2 socket for targetCellId="
                    << params.targetCellId << " (sourceCellId=" << params.sourceCellId
                    << "). Handover dropped — check AddX2Interface covers all cell pairs.");
        return;
    }
...
EpcX2::DoSendRlcSetupRequest(EpcX2SapProvider::RlcSetupRequest params)
{
    ...
    if (m_x2InterfaceSockets.find(params.targetCellId) == m_x2InterfaceSockets.end())
    {
        NS_LOG_WARN("[EpcX2] DoSendRlcSetupRequest: no X2 socket for targetCellId="
                    << params.targetCellId << " (sourceCellId=" << params.sourceCellId
                    << "). Request dropped.");
        return;
    }
