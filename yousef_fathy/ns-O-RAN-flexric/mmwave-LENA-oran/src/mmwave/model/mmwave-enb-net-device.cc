        ...
        switch (controlMessage->m_e2SmRcControlHeaderFormat1->ric_ControlAction_ID)
          {
            case RicControlMessage::Connected_Mode_Mobility_Control_Action_ID::Handover_Control: {
              ...
              uint64_t imsi = {0};
              size_t copy_sz = UEgnb->ran_UEID->size < sizeof(imsi) ? UEgnb->ran_UEID->size : sizeof(imsi);
              memcpy (&imsi, UEgnb->ran_UEID->buf, copy_sz);
  ...
  for (auto ue : ueMap)
    {
      ...
      // ── Real RSRP for serving cell (from m_l3rsrpMap) ────────────────────
      double rsrpThisCell = -200.0;
      if (m_l3rsrpMap.find (imsi) != m_l3rsrpMap.end () &&
          m_l3rsrpMap.at (imsi).find (m_cellId) != m_l3rsrpMap.at (imsi).end ())
        {
          rsrpThisCell = m_l3rsrpMap.at (imsi).at (m_cellId);
        }

      Ptr<L3RrcMeasurements> l3RrcMeasurementServing;
      Ptr<L3RrcMeasurements> l3RrcMeasurementServingRsrp;
      if (!indicationMessageHelper->IsOffline ())
        {
          // SINR + RSRP معاً في نفس الـ L3RRC measurement
          long rsrpThisCellEncoded = (long)(rsrpThisCell + 157.0);
          if (rsrpThisCellEncoded < 0) rsrpThisCellEncoded = 0;
          if (rsrpThisCellEncoded > 97) rsrpThisCellEncoded = 97;
          l3RrcMeasurementServing = L3RrcMeasurements::CreateL3RrcUeSpecificSinrServingWithRsrp (
              m_cellId, m_cellId, sinrThisCell, rsrpThisCellEncoded);
        }
      NS_LOG_DEBUG (Simulator::Now ().GetSeconds ()
      ...
      for (std::map<long double, uint16_t>::iterator it = --sortFlipMap.end ();
           it != --sortFlipMap.begin () && itIndex < nNeighbours; it--)
        {
          ...
          if (!indicationMessageHelper->IsOffline ())
            {
              // ابحث عن RSRP لهذه الـ neighbor cell
              long neighRsrpEncoded = 0;
              if (m_l3rsrpMap.find (imsi) != m_l3rsrpMap.end ())
                {
                  uint16_t realCellId = (uint16_t)std::abs ((long)cellId);
                  auto rsrpIt = m_l3rsrpMap.at (imsi).find (realCellId);
                  if (rsrpIt != m_l3rsrpMap.at (imsi).end ())
                    {
                      neighRsrpEncoded = (long)(rsrpIt->second + 157.0);
                      if (neighRsrpEncoded < 0) neighRsrpEncoded = 0;
                      if (neighRsrpEncoded > 97) neighRsrpEncoded = 97;
                    }
                }
              if (neighRsrpEncoded > 0)
                l3RrcMeasurementNeigh->AddNeighbourCellMeasurementWithRsrp (cellId, sinr, neighRsrpEncoded);
              else
      ...
      // ── Build neighbor RSRP measurement (L3neighRSRPListOf_UEID_Y_of_Cell_X) ─
      Ptr<L3RrcMeasurements> l3RrcMeasurementNeighRsrp;
      if (!indicationMessageHelper->IsOffline ())
        {
          l3RrcMeasurementNeighRsrp =
              L3RrcMeasurements::CreateL3RrcUeSpecificRsrpNeigh (m_cellId);

          if (m_l3rsrpMap.find (imsi) != m_l3rsrpMap.end ())
            {
              // Sort neighbors by RSRP (descending) same as SINR loop
              std::multimap<double, uint16_t> rsrpSortMap;
              for (auto &kv : m_l3rsrpMap.at (imsi))
                rsrpSortMap.insert ({kv.second, kv.first});

              uint16_t nRsrpNeigh = E2SM_REPORT_MAX_NEIGH;
              if (m_l3rsrpMap.at (imsi).size () < nRsrpNeigh)
                nRsrpNeigh = (uint16_t)(m_l3rsrpMap.at (imsi).size () - 1);

              int rIdx = 0;
              for (auto rit = rsrpSortMap.rbegin ();
                   rit != rsrpSortMap.rend () && rIdx < nRsrpNeigh; ++rit)
                {
                  long ncid = (long)rit->second;
                  if (ncid == (long)m_cellId) ncid *= -1;
long convertedNeighRsrp = (long)(rit->first + 157.0);
if (convertedNeighRsrp < 0) convertedNeighRsrp = 0;
if (convertedNeighRsrp > 97) convertedNeighRsrp = 97;
l3RrcMeasurementNeighRsrp->AddNeighbourCellMeasurement (ncid, convertedNeighRsrp);
                  NS_LOG_DEBUG (" neigh RSRP cell " << ncid << " = " << rit->first << " dBm");
                  rIdx++;
                }
            }
        }
...
// نضيف RSRP في الـ ueImsiComplete string عشان يوصل للـ xApp
indicationMessageHelper->AddCuCpUePmItem (ueImsiComplete, numDrb, 0,
      ...
      if (m_l3rsrpMap.find(imsi) != m_l3rsrpMap.end()) {
        for (auto& kv : m_l3rsrpMap.at(imsi)) {
          long ncid = (long)kv.first;
          long rsrpEnc = (long)(kv.second + 157.0);
          if (rsrpEnc < 0) rsrpEnc = 0;
          if (rsrpEnc > 97) rsrpEnc = 97;
          std::string rsrpNeighName = "L3neighRSRPListOf_UEID_" +
                                      std::to_string(imsi) +
                                      "_of_Cell_" + std::to_string(ncid);
          ueVal->AddItem<long>(rsrpNeighName.c_str(), rsrpEnc);
        }
      }

// RSRP reaches xApp via ueVal->AddItem directly (L3servingRSRP3gpp + L3neighRSRPListOf)
        }
    }
