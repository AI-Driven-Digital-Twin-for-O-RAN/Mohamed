...
async def start_simulation(request: Request):
    ...
    number_of_ues = int(form_data.get('N_Ues', 20))
    number_of_cells = int(form_data.get('N_LteEnbNodes', 1)) + int(form_data.get('N_MmWaveEnbNodes', 7))
    if not flags:
        # سيناريو Energy_Saving_with_load_balancing: قيم ثابتة 7 mmWave + 1 LTE = 8 خلايا، 20 UE
        if scenario and ("Energy_Saving_with_load_balancing" in scenario or "load_balancing_scenario" in scenario):
            number_of_ues = 20
            number_of_cells = 8
        else:
            number_of_ues = 0
