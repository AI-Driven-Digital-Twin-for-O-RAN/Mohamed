 ...
 *
 * EXTENDED FOR INTER-LTE HANDOVER PROOF-OF-CONCEPT
 */
...
/**
 * Scenario Zero - Extended for Inter-LTE Handover
 *
 * Architecture:
 * - 1 UE
 * - 2 LTE eNBs (Master Cells)
 * - 2 mmWave gNBs (Secondary Cells, each co-located with an LTE eNB)
 * - NSA mode preserved
 *
 * Handover:
 * - UE moves from LTE eNB 1 toward LTE eNB 2
 * - Inter-LTE handover occurs automatically based on measurements
 * - mmWave secondary cell behavior follows NSA rules
 */
...
main(int argc, char *argv[]) {
    // Enable detailed LTE RRC logging for handover verification
    ...
    // LTE Handover configuration - tuned for faster, more reliable handover
    ...
    Config::SetDefault("ns3::A3RsrpHandoverAlgorithm::Hysteresis", DoubleValue(1.5));
    Config::SetDefault("ns3::A3RsrpHandoverAlgorithm::TimeToTrigger", TimeValue(MilliSeconds(128)));
    ...
    NS_LOG_INFO("Bandwidth " << bandwidth << " centerFrequency " << double(centerFrequency)
                             << " isd_ue " << isd_ue << " isd_cell " << isd_cell 
    ...
    // TOPOLOGY FOR HANDOVER:
    // LTE eNB 1 at (1500, 2000) co-located with mmWave gNB 1
    // LTE eNB 2 at (2500, 2000) co-located with mmWave gNB 2
    // UE starts near LTE eNB 1 and moves toward LTE eNB 2
    
    Ptr <ListPositionAllocator> enbPositionAlloc = CreateObject<ListPositionAllocator>();
    
    // LTE eNB 1 position (source)
    enbPositionAlloc->Add(Vector(1500, 2000, 3));
    // LTE eNB 2 position (target)
    enbPositionAlloc->Add(Vector(2500, 2000, 3));
    // mmWave gNB 1 - co-located with LTE eNB 1
    enbPositionAlloc->Add(Vector(1500, 2000, 3));
    // mmWave gNB 2 - co-located with LTE eNB 2
    enbPositionAlloc->Add(Vector(2500, 2000, 3));
    ...
    // UE Mobility: Start near LTE eNB 1, move toward LTE eNB 2
    MobilityHelper uemobility;
    Ptr<ListPositionAllocator> uePositionAlloc = CreateObject<ListPositionAllocator>();
    
    // UE starts at (1400, 2000) - close to LTE eNB 1
    uePositionAlloc->Add(Vector(1400, 2000, 1.5));
    
    uemobility.SetPositionAllocator(uePositionAlloc);
    // UE moves in +X direction (toward LTE eNB 2) at 10 m/s
    uemobility.SetMobilityModel("ns3::ConstantVelocityMobilityModel");
    uemobility.Install(ueNodes);
    
    // Set velocity: 10 m/s in X direction
    ueNodes.Get(0)->GetObject<ConstantVelocityMobilityModel>()->SetVelocity(Vector(10.0, 0.0, 0.0));
    
    NS_LOG_UNCOND("UE initial position: " << ueNodes.Get(0)->GetObject<MobilityModel>()->GetPosition());
    NS_LOG_UNCOND("UE velocity: 10 m/s toward LTE eNB 2");
    ...
    // CRITICAL: AttachToClosestEnb is called ONLY ONCE for initial attachment
    mmwaveHelper->AttachToClosestEnb(mcUeDevs, mmWaveEnbDevs, lteEnbDevs);
    
    NS_LOG_UNCOND("Initial attachment complete - UE attached to closest LTE eNB");
    ...
    AnimationInterface anim("lte-handover-scenario.xml");
    anim.SetMaxPktsPerTraceFile(500000);
    
    // Color LTE eNBs in blue
    for (uint32_t i = 0; i < lteEnbNodes.GetN(); ++i) {
        anim.UpdateNodeDescription(lteEnbNodes.Get(i), "LTE-eNB");
        anim.UpdateNodeColor(lteEnbNodes.Get(i), 0, 0, 255);
        anim.UpdateNodeSize(lteEnbNodes.Get(i)->GetId(), 30, 30);
    }
    
    // Color mmWave gNBs in green
    for (uint32_t i = 0; i < mmWaveEnbNodes.GetN(); ++i) {
        anim.UpdateNodeDescription(mmWaveEnbNodes.Get(i), "mmWave-gNB");
        anim.UpdateNodeColor(mmWaveEnbNodes.Get(i), 0, 255, 0);
        anim.UpdateNodeSize(mmWaveEnbNodes.Get(i)->GetId(), 20, 20);
    }
    ...
    if (run) {
        ...
        NS_LOG_UNCOND("  1. UE initially attached to LTE eNB 1 (Cell ID 1)");
        NS_LOG_UNCOND("  2. UE moves toward LTE eNB 2 at 10 m/s");
        NS_LOG_UNCOND("  3. Inter-LTE handover from eNB 1 to eNB 2 should occur");
        NS_LOG_UNCOND("  4. Watch for 'Handover' messages in LteEnbRrc logs");
    ...
    NS_LOG_UNCOND("Check logs for handover events");
    NS_LOG_UNCOND("NetAnim file: lte-handover-scenario.xml");
    return 0;
}
