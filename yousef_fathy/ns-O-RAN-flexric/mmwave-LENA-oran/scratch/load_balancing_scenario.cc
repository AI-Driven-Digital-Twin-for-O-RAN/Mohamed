 ...
 *
 * Load Balancing xApp scenario (dense urban, 7 gNBs, 20 UEs):
 *   Cell 2 (center mmWave) intentionally overloaded with 8 UEs; neighbors carry 2 UEs each.
 *   Service differentiation: VoIP (UE1-6), Video streaming (UE7-11), Gaming (UE12-15),
 *   Best-effort browsing (UE16-20) — PRB usage varies per type to show PRB-based LB accuracy.
 *   KPIs logged to ~/load_balance_kpis.csv every 1s: PRB utilization, UE count, load variance,
 *   avg throughput, CDR. KPM_E2functionID=2, RC_E2functionID=3; handover mode NoAuto.
 */
...
// Per-UE speed (m/s) — mixed mobility: pedestrians in Cell 2 + vehicles crossing boundaries
// UE7 and UE8 are vehicles: offloaded from Cell 2, then traverse neighbour cells
static const double g_ue_speed_mps[20] = {
    ...
    2.0,  // UE 5  pedestrian
    1.3,  // UE 6  pedestrian
    18.0, // UE 7  vehicle — crosses cell boundaries after initial offload
    22.0, // UE 8  vehicle — crosses cell boundaries after initial offload
    3.5,  // UE 9  fast walker
    4.0,  // UE 10 fast walker / cyclist
    1.4,  // UE 11 pedestrian
    2.0,  // UE 12 pedestrian
    12.0, // UE 13 vehicle (slow)
    20.0, // UE 14 vehicle
    4.5,  // UE 15 cyclist
    25.0, // UE 16 vehicle
    3.0,  // UE 17 fast walker
    1.5,  // UE 18 pedestrian
    15.0, // UE 19 vehicle
    30.0  // UE 20 vehicle (fast)
};
...
// Load Balancing KPI tracking
static int g_total_handovers = 0;
static int g_total_drops     = 0;
static std::ofstream g_lbKpiCsv;
...
static ns3::GlobalValue g_simTime("simTime", "Simulation time in seconds",
                                   ns3::DoubleValue(120), ns3::MakeDoubleChecker<double>(0.1, 100000.0));
...
// ============================================================
// LogLoadBalancingKpis — logs 5 KPIs per cell every second
// KPI 1: PRB utilization (%) per cell
// KPI 2: UE count per cell
// KPI 3: Load variance across all cells (PRB-based)
// KPI 4: Average DL throughput per UE (bps)
// KPI 5: Call Drop Rate = drops / total HO attempts
// ============================================================
static void LogLoadBalancingKpis (NodeContainer* ueNodes)
{
  if (!g_lbKpiCsv.is_open ())
    {
      const char* home = std::getenv ("HOME");
      std::string path = (home && home[0])
                         ? std::string (home) + "/load_balance_kpis.csv"
                         : "load_balance_kpis.csv";
      g_lbKpiCsv.open (path, std::ios::out | std::ios::trunc);
      if (g_lbKpiCsv.is_open ())
        g_lbKpiCsv << "time_sec,cell_id,prb_util_pct,ue_count,"
                      "load_variance,avg_dl_throughput_bps,cdr\n";
    }
  if (!g_lbKpiCsv.is_open ()) return;

  double t = Simulator::Now ().GetSeconds ();

  // UE count per cell
  std::map<uint16_t, int> ues_per_cell;
  uint32_t nUes = ueNodes ? ueNodes->GetN () : 0;
  ...
  std::map<uint16_t, double> prb_util;
  for (auto& kv : g_real_prb_total)
    {
      uint16_t cid  = kv.first;
      int total     = kv.second;
      int used      = g_real_prb_used.count (cid) ? g_real_prb_used[cid] : 0;
      prb_util[cid] = (total > 0) ? (100.0 * used / total) : 0.0;
    }

  // Load variance across all cells (PRB-based)
  double sum = 0.0;
  for (auto& kv : prb_util) sum += kv.second;
  double mean = prb_util.empty () ? 0.0 : sum / prb_util.size ();
  double variance = 0.0;
  for (auto& kv : prb_util) variance += (kv.second - mean) * (kv.second - mean);
  if (!prb_util.empty ()) variance /= prb_util.size ();

  // Average DL throughput per UE
  double total_tp = 0.0;
  for (uint32_t u = 0; u < nUes; ++u)
    total_tp += GetDlBitrate (u + 1);
  double avg_tp = (nUes > 0) ? (total_tp / nUes) : 0.0;

  // CDR
  double cdr = (g_total_handovers > 0)
               ? ((double) g_total_drops / g_total_handovers)
               : 0.0;

  // Write one row per active cell
  for (auto& kv : prb_util)
    {
      uint16_t cid = kv.first;
      int nue      = ues_per_cell.count (cid) ? ues_per_cell[cid] : 0;
      g_lbKpiCsv << t << "," << cid << "," << kv.second << "," << nue << ","
                 << variance << "," << avg_tp << "," << cdr << "\n";
    }
  g_lbKpiCsv.flush ();
}
...
// ============================================
 ...
 // NS_LOG_UNCOND("RRC Measurement Configuration: A3Offset=0.5dB, Hysteresis=0dB, TTT=160ms");
  ...
  // ============================================================
  // Load Balancing scenario: fixed initial UE positions
  // Cell 2 (center mmWave) overloaded — 8 UEs (UE1-8)
  // Each neighbor cell gets exactly 2 UEs
  // ============================================================
  static const double UE_INIT_POS[20][2] = {
      {1021, 977},   // UE1  → Cell 2 (overloaded)
      {980,  978},   // UE2  → Cell 2
      {950,  930},   // UE3  → Cell 2
      {912,  1000},  // UE4  → Cell 2
      {1088, 1000},  // UE5  → Cell 2
      {950,  1070},  // UE6  → Cell 2
      {1050, 950},   // UE7  → Cell 2
      {1050, 1070},  // UE8  → Cell 2
      {823,  1320},  // UE9  → Cell 5
      {800,  1294},  // UE10 → Cell 5
      {685,  1008},  // UE11 → Cell 6
      {675,  950},   // UE12 → Cell 6
      {833,  796},   // UE13 → Cell 7
      {820,  670},   // UE14 → Cell 7
      {827,  1168},  // UE15 → Cell 8
      {1120, 690},   // UE16 → Cell 8
      {1325, 1018},  // UE17 → Cell 3
      {1256, 1046},  // UE18 → Cell 3
      {1230, 1255},  // UE19 → Cell 4
      {1160, 1350},  // UE20 → Cell 4
  };

  Ptr<ListPositionAllocator> uePositionAlloc = CreateObject<ListPositionAllocator>();
  for (uint32_t i = 0; i < nUeNodes; ++i)
    {
      double px = std::max (UE_BOUND_MIN, std::min (UE_BOUND_MAX, UE_INIT_POS[i][0]));
      double py = std::max (UE_BOUND_MIN, std::min (UE_BOUND_MAX, UE_INIT_POS[i][1]));
      uePositionAlloc->Add (Vector (px, py, 1.5));
      NS_LOG_UNCOND ("[LB-INIT] UE" << (i + 1) << " placed at (" << px << "," << py << ")");
    }
  ...
  // All UEs: standard boundary [500,1500] with realistic speeds
  for (uint32_t i = 0; i < ueNodes.GetN (); ++i)
    {
      ...
      double angle = randAngle->GetValue () * 2.0 * M_PI;
      double speed = (i < 20u) ? g_ue_speed_mps[i] : 1.5;
      mob->SetVelocity (Vector (speed * std::cos (angle), speed * std::sin (angle), 0.0));
      ...
      double firstChange = DIR_CHANGE_MIN_S +
                           (DIR_CHANGE_MAX_S - DIR_CHANGE_MIN_S) * randAngle->GetValue ();
      Simulator::Schedule (Seconds (firstChange),
                           &BoundedMobilityDirectionChange, i, ueNodes.Get (i), randAngle, randSpeed);
    }

  NS_LOG_UNCOND ("[LB-MOBILITY] All 20 UEs: random walk, realistic speeds, bounded to ["
                 << UE_BOUND_MIN << "," << UE_BOUND_MAX << "] m");
  // ============================================
  ...
  // ── Service-differentiated traffic (4 types) ──────────────────────────────────────────
  // VoIP    (UE 1-6,  idx 0-5):  UDP 20ms/160B  ~64 kbps  — low PRB
  // Video   (UE 7-11, idx 6-10): UDP 5ms/1400B  ~2.2 Mbps — high PRB
  // Gaming  (UE12-15, idx 11-14):UDP 16ms/512B  ~256 kbps — medium PRB
  // Browsing(UE16-20, idx 15-19):TCP OnOff 1Mbps           — variable PRB
  // Purpose: PRB-based load measurement is more accurate than UE-count-based
  NS_LOG_UNCOND("=== INSTALLING TRAFFIC APPLICATIONS (LB scenario: VoIP/Video/Gaming/Browse) ===");
  ...
  const uint32_t nVoipUes   = 6;   // idx 0-5
  const uint32_t nVideoUes  = 5;   // idx 6-10
  const uint32_t nGamingUes = 4;   // idx 11-14
  // idx 15-19: best-effort browsing

  // Port per service type
  const uint16_t voipPort   = 5060;
  const uint16_t videoPort  = 1234;
  const uint16_t gamingPort = 2000;
  const uint16_t browsePort = 80;
  ...
  for (uint32_t u = 0; u < ueNodes.GetN(); ++u) {
      ...
      bool isVoip   = (u < nVoipUes);
      bool isVideo  = (!isVoip && u < nVoipUes + nVideoUes);
      bool isGaming = (!isVoip && !isVideo && u < nVoipUes + nVideoUes + nGamingUes);
      ...
      const char* svcName = isVoip ? "VoIP" : isVideo ? "Video" : isGaming ? "Gaming" : "Browsing";
      NS_LOG_UNCOND("  [OK] UE" << (u+1) << " [" << svcName << "] IP=" << ueAddr);

      // DL client (remoteHost → UE)
      if (isVoip) {
          // VoIP: G.711-like, 64 kbps, 20ms interval, 160 B
          ...
          // Video streaming: ~2.2 Mbps, 5ms interval, 1400 B
          UdpClientHelper dlClient (ueAddr, sinkPort);
          ...
          // Gaming: ~256 kbps, 16ms interval, 512 B
          UdpClientHelper dlClient (ueAddr, sinkPort);
          ...
          // Best-effort browsing: TCP OnOff, variable ~1 Mbps
          OnOffHelper dlClient ("ns3::TcpSocketFactory",
                                InetSocketAddress (ueAddr, sinkPort));
          dlClient.SetAttribute ("OnTime",  StringValue ("ns3::ExponentialRandomVariable[Mean=1.0]"));
          dlClient.SetAttribute ("OffTime", StringValue ("ns3::ExponentialRandomVariable[Mean=1.5]"));
          dlClient.SetAttribute ("DataRate", DataRateValue (DataRate ("1Mbps")));
          dlClient.SetAttribute ("PacketSize", UintegerValue (1024));
          clientApp.Add (dlClient.Install (remoteHost));
      }

      // UL sink on remoteHost (unique port per UE)
      ...
      // UL client (UE → remoteHost) — symmetric for VoIP/Gaming, small feedback for Video/Browse
      UdpClientHelper ulClient (remoteHostAddr, ulPort);
      if (isVoip) {
          ulClient.SetAttribute ("Interval",   TimeValue (MilliSeconds (20)));
          ulClient.SetAttribute ("PacketSize", UintegerValue (160));
      } else if (isVideo) {
          ulClient.SetAttribute ("Interval",   TimeValue (MilliSeconds (100)));
          ulClient.SetAttribute ("PacketSize", UintegerValue (64));
      } else if (isGaming) {
          ulClient.SetAttribute ("Interval",   TimeValue (MilliSeconds (16)));
...
enbDev2->GetMac()->TraceConnectWithoutContext(
  ...
  // ── Schedule Load Balancing KPI logging every 1 second ──────────────────
  for (double t = 1.0; t < simTime; t += 1.0)
    {
      Simulator::Schedule (Seconds (t), &LogLoadBalancingKpis, &ueNodes);
    }
  NS_LOG_UNCOND ("[LB-KPI] Load Balancing KPIs will be logged to ~/load_balance_kpis.csv every 1s");
...
// Close handover log
  ...
  if (g_lbKpiCsv.is_open ()) {
      g_lbKpiCsv.close ();
      NS_LOG_UNCOND("Load Balancing KPI CSV closed: load_balance_kpis.csv");
  }
