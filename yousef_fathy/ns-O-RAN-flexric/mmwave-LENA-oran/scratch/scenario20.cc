 ...
 * FIXES:
 * 1. Better SECONDARY_CHANGE logging with real metrics
 * 2. GUI shows BOTH LTE and mmWave cells
 */
...
std::map<uint64_t, uint16_t> ueCurrentMmWaveCell;
std::map<uint64_t, bool> ueInHandover;
std::map<uint64_t, bool> ueInitialConnectionDone;
std::ofstream hoLogFile;
std::ofstream posLogFile;
std::ofstream metricsLogFile;
std::ofstream lteGuiLogFile; // NEW: Separate file for LTE tracking
...
// LTE to mmWave Cell Mapping
std::map<uint16_t, uint16_t> lteToMmWaveMapping = {
    {1, 4},
    {2, 5},
    {3, 6}
};

// Global references for Secondary Cell Change
Ptr<MmWaveHelper> globalMmwaveHelper;
NetDeviceContainer globalMcUeDevs;
NetDeviceContainer globalMmWaveEnbDevs;
NetDeviceContainer globalLteEnbDevs;
NodeContainer globalUeNodes; // NEW: For calculating metrics
...
void ClearFile(std::string Filename, uint64_t m_startTime) {
    ...
    else if (Filename == "lte_ue_tracking.txt") {
        outFile1 << "timestamp,imsi,lte_cell,mmwave_cell,simid" << std::endl;
    }
    outFile1.close();
}
...
void PrintGnuplottableEnbListToFile(uint64_t m_startTime, NodeContainer lteEnbNodes, NodeContainer mmWaveEnbNodes) {
    ...
    for (uint32_t i = 0; i < lteEnbNodes.GetN(); i++) {
        ...
        if (enbdev) {
            ...
            outFile1 << timestamp << "," << cell_id << "," << pos.x << "," << pos.y << ","
                     << m_startTime << "," << connected_ues << "," << "30" << std::endl;
            outFile1.close();
        }
    }
    
    // Write mmWave gNBs with correct UE associations
    for (uint32_t i = 0; i < mmWaveEnbNodes.GetN(); i++) {
        ...
        if (mmdev) {
            ...
            // Count connected UEs using ueCurrentMmWaveCell
            int connected_ues = 0;
            for (uint64_t imsi = 1; imsi <= 20; imsi++) {
                if (ueCurrentMmWaveCell.find(imsi) != ueCurrentMmWaveCell.end() && 
                    ueCurrentMmWaveCell[imsi] == cell_id) {
                    connected_ues++;
                }
            }
            ...
            outFile2 << timestamp << "," << cell_id << "," << pos.x << "," << pos.y << ","
                     << m_startTime << "," << "false" << "," << connected_ues << "," 
                     << "100" << "," << "0" << std::endl;
            outFile2.close();
        }
    }
...
void PrintPosition(Ptr<Node> node, int iterator, std::string Filename, uint64_t m_startTime) {
    ...
    for (int j = 0; j < nDevs; j++) {
        ...
        if (mcuedev) {
            ...
            // Use mmWave cell for GUI display
            int serving_cell = ue_assoc_list[imsi - 1];
            if (serving_cell == 0) {
                serving_cell = 4; // Default mmWave cell
            }
...
// NEW: Function to update LTE tracking file
void UpdateLteTracking(uint64_t m_startTime) {
    uint64_t timestamp = m_startTime + (uint64_t) Simulator::Now().GetMilliSeconds();
    std::string filename = "lte_ue_tracking.txt";
    ...
    for (uint64_t imsi = 1; imsi <= 20; imsi++) {
        if (ueCurrentCell.find(imsi) != ueCurrentCell.end() && 
            ueCurrentMmWaveCell.find(imsi) != ueCurrentMmWaveCell.end()) {
            
            uint16_t lte_cell = ueCurrentCell[imsi];
            uint16_t mmwave_cell = ueCurrentMmWaveCell[imsi];
            
            outFile << timestamp << "," << imsi << "," << lte_cell << "," 
                    << mmwave_cell << "," << m_startTime << std::endl;
        }
    }
...
// IMPROVED: Function for Secondary Cell Change with real metrics
void TriggerSecondaryCellChange(uint64_t imsi, uint16_t newLteCell, const std::vector<Vector>& cellPositions) {
    NS_LOG_UNCOND("\n🔄═══════════════════════════════════════");
    NS_LOG_UNCOND("   SECONDARY CELL CHANGE INITIATED");
    NS_LOG_UNCOND("═══════════════════════════════════════");
    
    if (lteToMmWaveMapping.find(newLteCell) == lteToMmWaveMapping.end()) {
        NS_LOG_ERROR("⚠️ No mmWave mapping for LTE Cell " << newLteCell);
        return;
    }
    
    uint16_t oldMmWaveCell = ueCurrentMmWaveCell[imsi];
    uint16_t newMmWaveCell = lteToMmWaveMapping[newLteCell];
    
    NS_LOG_UNCOND("📱 UE-" << imsi);
    NS_LOG_UNCOND("📡 LTE Cell: " << newLteCell);
    NS_LOG_UNCOND("🔄 mmWave: " << oldMmWaveCell << " → " << newMmWaveCell);
    
    // Update tracking
    ueCurrentMmWaveCell[imsi] = newMmWaveCell;
    if (imsi >= 1 && imsi <= 20) {
        ue_assoc_list[imsi - 1] = newMmWaveCell;
    }
    
    NS_LOG_UNCOND("✅ Secondary Cell Changed Successfully!");
    NS_LOG_UNCOND("🎨 GUI Updated: UE-" << imsi << " → mmWave Cell " << newMmWaveCell);
    NS_LOG_UNCOND("═══════════════════════════════════════\n");
    
    // Calculate real metrics instead of -999
    double now = Simulator::Now().GetSeconds();
    uint32_t ueIndex = imsi - 1;
    
    if (ueIndex < globalUeNodes.GetN()) {
        Ptr<Node> ueNode = globalUeNodes.Get(ueIndex);
        
        // Calculate RSRP for new cell
        double rsrp = CalculateRsrp(ueNode, newLteCell, cellPositions);
        ...
        metricsLogFile << imsi << "," << now << "," << ueIndex << "," << newMmWaveCell << ","
                       << rsrp << "," << sinr << "," << rsrq << "," << cqi << ","
                       << secondCellRsrp << "," << secondCellSinr << "," << nrxlev1 << "," << nqual1 << ","
                       << speed << ",0,0,100,0," << rsrq << ",SECONDARY_CHANGE" << std::endl;
    } else {
        // Fallback to -999 only if UE not found
        metricsLogFile << imsi << "," << now << "," << (imsi-1) << "," << newMmWaveCell << ","
                       << "-999,-999,-999,0,0,0,0,0,0,0,0,100,0,0,SECONDARY_CHANGE" << std::endl;
    }
}
...
void NotifyHandoverStart(std::string context, uint64_t imsi, 
  ...
  NS_LOG_UNCOND("📡 LTE HO: Cell " << cellId << " → Cell " << targetCellId);
  
  uint16_t oldMmWave = ueCurrentMmWaveCell[imsi];
  uint16_t newMmWave = lteToMmWaveMapping[targetCellId];
  NS_LOG_UNCOND("🔄 mmWave will change: " << oldMmWave << " → " << newMmWave);
  NS_LOG_UNCOND("════════════════════════════════════════");
...
void NotifyHandoverEnd(std::string context, uint64_t imsi, 
  ...
  uint16_t oldLteCell = ueCurrentCell[imsi];
  ueCurrentCell[imsi] = cellId;
  ueInHandover[imsi] = false;
  
  NS_LOG_UNCOND("✅ LTE HANDOVER COMPLETED - UE-" << imsi);
  NS_LOG_UNCOND("⏱️  Time: " << now << "s");
  NS_LOG_UNCOND("📡 LTE: Cell " << oldLteCell << " → Cell " << cellId);
  
  hoLogFile << now << "," << imsi << "," << ueHoCount[imsi] << "," 
            << oldLteCell << "," << cellId << ",COMPLETE" << std::endl;
  
  // Trigger Secondary Cell Change with cell positions
  if (oldLteCell != cellId && oldLteCell != 0) {
      NS_LOG_UNCOND("🔄 Inter-eNB HO detected! Scheduling Secondary Cell Change...");
      ...
      Simulator::Schedule(MilliSeconds(100), &TriggerSecondaryCellChange, imsi, cellId, cellPositions);
  }
...
void NotifyConnectionEstablished(std::string context, uint64_t imsi, 
                                uint16_t cellId, uint16_t rnti) {
  if (ueInitialConnectionDone[imsi]) {
      return;
  }
  
  ueCurrentCell[imsi] = cellId;
  ueInitialConnectionDone[imsi] = true;
  
  if (lteToMmWaveMapping.find(cellId) == lteToMmWaveMapping.end()) {
      NS_LOG_ERROR("⚠️ No mmWave mapping for LTE Cell " << cellId);
      return;
  }
  
  uint16_t mmWaveCell = lteToMmWaveMapping[cellId];
  ueCurrentMmWaveCell[imsi] = mmWaveCell;
  
  if (imsi >= 1 && imsi <= 20) {
    ue_assoc_list[imsi - 1] = mmWaveCell;
    
    NS_LOG_UNCOND("🔗 UE-" << imsi << " INITIAL CONNECTION");
    NS_LOG_UNCOND("   LTE Cell: " << cellId);
    NS_LOG_UNCOND("   mmWave Cell: " << mmWaveCell);
    NS_LOG_UNCOND("   Time: " << Simulator::Now().GetSeconds() << "s");
    NS_LOG_UNCOND("   GUI: ue_assoc_list[" << (imsi-1) << "] = " << mmWaveCell);
    NS_LOG_UNCOND("   ✅ Connection setup complete!\n");
  }
}

void NotifyMmWaveConnectionReconfiguration(std::string context, uint64_t imsi,
                                          uint16_t cellId, uint16_t rnti) {
  if (!ueInitialConnectionDone[imsi]) {
      return;
  }
  ...
  if (cellId < 4 || cellId > 6) {
      return;
  }
  
  NS_LOG_UNCOND("🔄 mmWave Reconfiguration: UE-" << imsi << " → Cell " << cellId 
                << " at t=" << now << "s");
  
  if (imsi >= 1 && imsi <= 20) {
    ue_assoc_list[imsi - 1] = cellId;
    ueCurrentMmWaveCell[imsi] = cellId;
  }
}
...
void LogUeState(Ptr<Node> ueNode, uint32_t ueId) {
  ...
  if ((int)now % 10 == 0) {
    uint64_t imsi = ueId + 1;
    uint16_t lteCell = ueCurrentCell[imsi];
    uint16_t mmWaveCell = ueCurrentMmWaveCell[imsi];
    NS_LOG_UNCOND("📊 t=" << now << "s | UE-" << imsi << ": Pos=(" 
                  << (int)pos.x << "," << (int)pos.y << ") LTE=" << lteCell 
                  << " mmWave=" << mmWaveCell);
  }
...
void LogDetailedMetrics(Ptr<Node> ueNode, uint32_t ueId, Ptr<PacketSink> sink, 
  ...
  uint16_t displayCell = ueCurrentMmWaveCell[imsi];
  if (displayCell == 0 || displayCell < 4 || displayCell > 6) {
      uint16_t lteCell = ueCurrentCell[imsi];
      if (lteToMmWaveMapping.find(lteCell) != lteToMmWaveMapping.end()) {
          displayCell = lteToMmWaveMapping[lteCell];
          ueCurrentMmWaveCell[imsi] = displayCell;
      } else {
          displayCell = 4;
      }
  }
  
  double rsrp = CalculateRsrp(ueNode, ueCurrentCell[imsi], cellPositions);
  double rsrq = rsrp - 12.0 - (rand() % 6);
  ...
  metricsLogFile << imsi << "," << now << "," << ueNode->GetId() << "," << displayCell << ","
                 << rsrp << "," << sinr << "," << rsrq << "," << cqi << ","
                 << secondCellRsrp << "," << secondCellSinr << "," << nrxlev1 << "," << nqual1 << ","
...
int main(int argc, char* argv[]) {
  ...
  metricsLogFile << "SessionID,ElapsedTime,Node,CellID,Level,SNR,Qual,CQI,"
                 ...
                 << "DL_bitrate,UL_bitrate,BANDWIDTH,Bearing,RSRQ,Event" << std::endl;
  
  lteGuiLogFile.open("lte_ue_tracking.txt");
  lteGuiLogFile << "timestamp,imsi,lte_cell,mmwave_cell,simid" << std::endl;
  
  NS_LOG_UNCOND("\n╔═══════════════════════════════════════════════╗");
  NS_LOG_UNCOND("║   Author: Youssef Fathy                       ║");
  NS_LOG_UNCOND("║   MULTI-UE MMWAVE HANDOVER SIMULATION         ║");
  NS_LOG_UNCOND("║   ✅ ALL FIXES APPLIED                        ║");
  NS_LOG_UNCOND("║   🎨 GUI shows LTE + mmWave tracking          ║");
  NS_LOG_UNCOND("║   📊 Real metrics in SECONDARY_CHANGE         ║");
  NS_LOG_UNCOND("╚═══════════════════════════════════════════════╝");
  ...
  for (uint32_t i = 0; i < numLteEnbs && i < cellPositions.size(); i++) {
    ...
    NS_LOG_UNCOND("   LTE eNB-" << (i+1) << " (Cell " << (i+1) << ") at (" 
                  << cellPositions[i].x << ", " << cellPositions[i].y << ")");
  }

  for (uint32_t i = 0; i < numMmWaveEnbs && i < cellPositions.size(); i++) {
    enbPosAlloc->Add(cellPositions[i]);
    NS_LOG_UNCOND("   mmWave gNB-" << (i+1) << " (Cell " << (i+4) << ") at (" 
                  << cellPositions[i].x << ", " << cellPositions[i].y << ")");
  }

  NS_LOG_UNCOND("\n📋 LTE-mmWave Cell Mapping:");
  for (auto& pair : lteToMmWaveMapping) {
      NS_LOG_UNCOND("   LTE Cell " << pair.first << " ↔ mmWave Cell " << pair.second);
  }
  ...
  globalMmwaveHelper = mmwaveHelper;
  globalMcUeDevs = mcUeDevs;
  globalMmWaveEnbDevs = mmWaveEnbDevs;
  globalLteEnbDevs = lteEnbDevs;
  globalUeNodes = ueNodes;
  ...
  // mmWave RRC callbacks
  Config::Connect("/NodeList/*/DeviceList/*/LteUeRrc/ConnectionReconfiguration",
                  MakeCallback(&NotifyMmWaveConnectionReconfiguration));
  ...
  ClearFile("lte_ue_tracking.txt", t_startTime_simid);
  ...
  for (int i = 0; i < numPrints; i++) {
    ...
    Simulator::Schedule(Seconds(i * simTime / numPrints), &UpdateLteTracking, t_startTime_simid);
  ...
  NS_LOG_UNCOND("   - youssef_fathy_ue_metrics.csv (with real SECONDARY_CHANGE metrics)");
  NS_LOG_UNCOND("   - youssef_fathy_mmwave_simulation.xml");
  NS_LOG_UNCOND("\n🌐 GUI Files (for http://127.0.0.1:8000/):");
  NS_LOG_UNCOND("   - ue_position.txt (mmWave cell display)");
  NS_LOG_UNCOND("   - lte_ue_tracking.txt (LTE cell tracking - NEW!)");
  NS_LOG_UNCOND("   - gnbs.txt (mmWave cells with UE count)");
  NS_LOG_UNCOND("   - enbs.txt (LTE cells with UE count)");
  NS_LOG_UNCOND("═══════════════════════════════════════════════\n");
