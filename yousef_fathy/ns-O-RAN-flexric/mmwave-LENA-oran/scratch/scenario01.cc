...
/* *
 * HANDOVER DEMO - Orange ns-O-RAN Repository
 * 6 UEs with GUARANTEED movement across 3 mmWave gNBs
 * Uses WaypointMobilityModel for reliable NetAnim visualization
 */
...
NotifyHandoverStartUe(std::string context, uint64_t imsi, uint16_t cellId, uint16_t rnti, uint16_t targetCellId)
{
    ...
    std::cout << "║ Source Cell: " << cellId << "\n";
    std::cout << "║ Target Cell: " << targetCellId << "\n";
    std::cout << "║ RNTI:        " << rnti << "\n";
    std::cout << "╚═══════════════════════════════════════╝\n" << std::endl;
}
...
void
LogUePosition(NodeContainer ueNodes, uint32_t ueIndex)
{
    ...
    std::cout << "[" << std::fixed << std::setprecision(1) << Simulator::Now().GetSeconds() 
              << "s] UE" << (ueIndex + 1) << " (IMSI " << (ueIndex + 1) << ") Position: ("
...
static ns3::GlobalValue g_reducedPmValues("reducedPmValues",
                                          "If true, use a subset of the pm containers",
...
static ns3::GlobalValue mmWave_nodes("N_MmWaveEnbNodes",
                                     "Number of mmWaveNodes",
                                     ns3::UintegerValue(3),
                                     ns3::MakeUintegerChecker<uint8_t>());
...
main(int argc, char* argv[])
{
    ...
    std::cout << "\n╔════════════════════════════════════════════════╗\n";
    std::cout << "║  🎯 HANDOVER DEMONSTRATION SCENARIO           ║\n";
    std::cout << "╠════════════════════════════════════════════════╣\n";
    std::cout << "║ Handover Mode:     " << std::setw(25) << std::left << handoverMode << "║\n";
    std::cout << "║ SINR Difference:   " << std::setw(25) << std::left << hoSinrDifference
              << "║\n";
    std::cout << "║ Outage Threshold:  " << std::setw(25) << std::left << outageThreshold
              << "║\n";
    std::cout << "╚════════════════════════════════════════════════╝\n\n";
    ...
    std::cout << "╠════════════════════════════════════════════════╣\n";
    std::cout << "║ LTE eNBs:     " << std::setw(30) << std::left
              << (int)nLteEnbNodes << "║\n";
    std::cout << "║ mmWave gNBs:  " << std::setw(30) << std::left
              << (int)nMmWaveEnbNodes << "║\n";
    std::cout << "║ UEs:          " << std::setw(30) << std::left << (int)nUeNodes << "║\n";
    std::cout << "║ Cell spacing: " << std::setw(30) << std::left << isd_cell << "║\n";
    std::cout << "╚════════════════════════════════════════════════╝\n\n";
    ...
    // create LTE, mmWave eNB nodes and UE nodes
    ...
    // ═══════════════════════════════════════════════════════════
    // CELL POSITIONS: Triangular layout
    // ═══════════════════════════════════════════════════════════
    ...
    // mmWave gNB positions in triangular pattern
    ...
    Vector gnb2Pos =
        Vector(centerPosition.x + isd_cell * cos(angle1), centerPosition.y + isd_cell * sin(angle1), 10);
    enbPositionAlloc->Add(gnb2Pos);

    double angle2 = 4.0 * M_PI / 3.0; // 240 degrees
    Vector gnb3Pos =
        Vector(centerPosition.x + isd_cell * cos(angle2), centerPosition.y + isd_cell * sin(angle2), 10);
    enbPositionAlloc->Add(gnb3Pos);

    std::cout << "╔════════════════════════════════════════════════╗\n";
    std::cout << "║  🗺️  CELL POSITIONS                           ║\n";
    std::cout << "╠════════════════════════════════════════════════╣\n";
    std::cout << "║ LTE eNB (Cell 1): " << std::setw(26) << std::left
              << ("(" + std::to_string((int)centerPosition.x) + ", " +
                  std::to_string((int)centerPosition.y) + ")")
              << "║\n";
    std::cout << "║ gNB1 (Cell 2):    " << std::setw(26) << std::left
              << ("(" + std::to_string((int)gnb1Pos.x) + ", " + std::to_string((int)gnb1Pos.y) +
                  ")")
              << "║\n";
    std::cout << "║ gNB2 (Cell 3):    " << std::setw(26) << std::left
              << ("(" + std::to_string((int)gnb2Pos.x) + ", " + std::to_string((int)gnb2Pos.y) +
                  ")")
              << "║\n";
    std::cout << "║ gNB3 (Cell 4):    " << std::setw(26) << std::left
              << ("(" + std::to_string((int)gnb3Pos.x) + ", " + std::to_string((int)gnb3Pos.y) +
                  ")")
              << "║\n";
    std::cout << "╚════════════════════════════════════════════════╝\n\n";
    ...
    // ═══════════════════════════════════════════════════════════
    // UE MOBILITY: WaypointMobilityModel for GUARANTEED movement
    // ═══════════════════════════════════════════════════════════
    std::cout << "╔════════════════════════════════════════════════╗\n";
    std::cout << "║  🚶 UE MOBILITY PATHS                         ║\n";
    std::cout << "╠════════════════════════════════════════════════╣\n";
    ...
    // UE 0: Start near gNB1, move toward gNB2
    Ptr<WaypointMobilityModel> ueMob0 = ueNodes.Get(0)->GetObject<WaypointMobilityModel>();
    Vector ue0Start = Vector(gnb1Pos.x - 80, gnb1Pos.y + 40, 1.5);
    Vector ue0End = Vector(gnb2Pos.x + 60, gnb2Pos.y - 30, 1.5);
    ueMob0->AddWaypoint(Waypoint(Seconds(0), ue0Start));
    ueMob0->AddWaypoint(Waypoint(Seconds(45), ue0End));
    std::cout << "║ UE1: (" << std::setw(4) << (int)ue0Start.x << "," << std::setw(4)
              << (int)ue0Start.y << ") → (" << std::setw(4) << (int)ue0End.x << "," << std::setw(4)
              << (int)ue0End.y << ") Cell2→3 ║\n";

    // UE 1: Start near gNB1, move toward gNB2
    Ptr<WaypointMobilityModel> ueMob1 = ueNodes.Get(1)->GetObject<WaypointMobilityModel>();
    Vector ue1Start = Vector(gnb1Pos.x - 50, gnb1Pos.y - 60, 1.5);
    Vector ue1End = Vector(gnb2Pos.x + 40, gnb2Pos.y - 60, 1.5);
    ueMob1->AddWaypoint(Waypoint(Seconds(0), ue1Start));
    ueMob1->AddWaypoint(Waypoint(Seconds(45), ue1End));
    std::cout << "║ UE2: (" << std::setw(4) << (int)ue1Start.x << "," << std::setw(4)
              << (int)ue1Start.y << ") → (" << std::setw(4) << (int)ue1End.x << "," << std::setw(4)
              << (int)ue1End.y << ") Cell2→3 ║\n";

    // UE 2: Start near gNB2, move toward gNB3
    Ptr<WaypointMobilityModel> ueMob2 = ueNodes.Get(2)->GetObject<WaypointMobilityModel>();
    Vector ue2Start = Vector(gnb2Pos.x + 60, gnb2Pos.y - 40, 1.5);
    Vector ue2End = Vector(gnb3Pos.x - 50, gnb3Pos.y + 60, 1.5);
    ueMob2->AddWaypoint(Waypoint(Seconds(0), ue2Start));
    ueMob2->AddWaypoint(Waypoint(Seconds(45), ue2End));
    std::cout << "║ UE3: (" << std::setw(4) << (int)ue2Start.x << "," << std::setw(4)
              << (int)ue2Start.y << ") → (" << std::setw(4) << (int)ue2End.x << "," << std::setw(4)
              << (int)ue2End.y << ") Cell3→4 ║\n";

    // UE 3: Start near gNB2, move toward gNB3
    Ptr<WaypointMobilityModel> ueMob3 = ueNodes.Get(3)->GetObject<WaypointMobilityModel>();
    Vector ue3Start = Vector(gnb2Pos.x + 30, gnb2Pos.y - 70, 1.5);
    Vector ue3End = Vector(gnb3Pos.x - 30, gnb3Pos.y + 40, 1.5);
    ueMob3->AddWaypoint(Waypoint(Seconds(0), ue3Start));
    ueMob3->AddWaypoint(Waypoint(Seconds(45), ue3End));
    std::cout << "║ UE4: (" << std::setw(4) << (int)ue3Start.x << "," << std::setw(4)
              << (int)ue3Start.y << ") → (" << std::setw(4) << (int)ue3End.x << "," << std::setw(4)
              << (int)ue3End.y << ") Cell3→4 ║\n";

    // UE 4: Start near gNB3, move toward gNB1
    Ptr<WaypointMobilityModel> ueMob4 = ueNodes.Get(4)->GetObject<WaypointMobilityModel>();
    Vector ue4Start = Vector(gnb3Pos.x - 50, gnb3Pos.y + 70, 1.5);
    Vector ue4End = Vector(gnb1Pos.x + 40, gnb1Pos.y - 60, 1.5);
    ueMob4->AddWaypoint(Waypoint(Seconds(0), ue4Start));
    ueMob4->AddWaypoint(Waypoint(Seconds(45), ue4End));
    std::cout << "║ UE5: (" << std::setw(4) << (int)ue4Start.x << "," << std::setw(4)
              << (int)ue4Start.y << ") → (" << std::setw(4) << (int)ue4End.x << "," << std::setw(4)
              << (int)ue4End.y << ") Cell4→2 ║\n";

    // UE 5: Start near gNB3, move toward gNB1
    Ptr<WaypointMobilityModel> ueMob5 = ueNodes.Get(5)->GetObject<WaypointMobilityModel>();
    Vector ue5Start = Vector(gnb3Pos.x + 60, gnb3Pos.y + 50, 1.5);
    Vector ue5End = Vector(gnb1Pos.x - 50, gnb1Pos.y + 40, 1.5);
    ueMob5->AddWaypoint(Waypoint(Seconds(0), ue5Start));
    ueMob5->AddWaypoint(Waypoint(Seconds(45), ue5End));
    std::cout << "║ UE6: (" << std::setw(4) << (int)ue5Start.x << "," << std::setw(4)
              << (int)ue5Start.y << ") → (" << std::setw(4) << (int)ue5End.x << "," << std::setw(4)
              << (int)ue5End.y << ") Cell4→2 ║\n";

    std::cout << "╚════════════════════════════════════════════════╝\n\n";

    // Install mmWave, lte, mc Devices
    ...
    // Schedule periodic UE position logging
    for (uint32_t u = 0; u < ueNodes.GetN(); ++u)
    {
        for (double t = 5.0; t < simTime; t += 5.0)
        {
            Simulator::Schedule(Seconds(t), &LogUePosition, ueNodes, u);
        }
    }
    ...
    anim.UpdateNodeDescription(mmWaveEnbNodes.Get(0), "gNB1");
    anim.UpdateNodeDescription(mmWaveEnbNodes.Get(1), "gNB2");
    anim.UpdateNodeDescription(mmWaveEnbNodes.Get(2), "gNB3");
    ...
    anim.UpdateNodeColor(mmWaveEnbNodes.Get(1), 255, 0, 0); // Red for gNB2
    anim.UpdateNodeColor(mmWaveEnbNodes.Get(2), 255, 0, 0); // Red for gNB3
    for (uint32_t i = 0; i < ueNodes.GetN(); ++i)
    {
        anim.UpdateNodeColor(ueNodes.Get(i), 0, 0, 255); // Blue for UEs
    }

    // Set node sizes for visibility
    anim.UpdateNodeSize(lteEnbNodes.Get(0)->GetId(), 8, 8);
    for (uint32_t i = 0; i < mmWaveEnbNodes.GetN(); ++i)
    {
        anim.UpdateNodeSize(mmWaveEnbNodes.Get(i)->GetId(), 6, 6);
    }
    for (uint32_t i = 0; i < ueNodes.GetN(); ++i)
    {
        anim.UpdateNodeSize(ueNodes.Get(i)->GetId(), 4, 4);
    }
    ...
    std::cout << "╔════════════════════════════════════════════════╗\n";
    std::cout << "║  ▶️  STARTING SIMULATION                      ║\n";
    std::cout << "╠════════════════════════════════════════════════╣\n";
    std::cout << "║ Duration: " << std::setw(35) << std::left << (simTime) << "║\n";
    std::cout << "║ Watch for handover messages below...          ║\n";
    std::cout << "╚════════════════════════════════════════════════╝\n\n";
    ...
    std::cout << "║ • scenario0_handover.xml (NetAnim)             ║\n";
    std::cout << "║ • ue_position.txt (UE trajectories)            ║\n";
    std::cout << "║ • gnbs.txt (gNB info)                          ║\n";
    std::cout << "║                                                ║\n";
    std::cout << "║ To visualize:                                  ║\n";
    std::cout << "║   netanim scenario0_handover.xml               ║\n";
    std::cout << "╚════════════════════════════════════════════════╝\n";
