#!/bin/bash
# ══════════════════════════════════════════════════════════════════════
#  gru.sh  —  GRU O-RAN Full Launcher
#  After any reboot, just run:  bash /home/omar_farouk/open-ran-clean/gru.sh
#
...
#
#  Optional args:   bash gru.sh [simTime] [nUEs] [nCells]
...
XAPP_GRU="/home/omar_farouk/open-ran-clean/yousef_fathy/flexric/build/examples/xApp/c/handover_gru/xapp_handover_gru"
GRU_PY="/home/omar_farouk/open-ran-clean/yousef_fathy/HANDOVER_xApp_Test/gru_xapp.py"
NS3="/home/omar_farouk/open-ran-clean/yousef_fathy/ns-O-RAN-flexric/mmwave-LENA-oran/ns3"
PUSHER="/home/omar_farouk/open-ran-clean/yousef_fathy/ns-O-RAN-flexric/mmwave-LENA-oran/sim_data_pusher.py"
CONTROLLER="$GUI_V2/controller.py"

echo ""
echo "╔══════════════════════════════════════════════════╗"
echo "║         GRU O-RAN Simulator — LAUNCHER           ║"
...
MISSING=0
for F in "$FLEXRIC_BIN" "$XAPP_GRU" "$GRU_PY" "$NS3" "$PUSHER" "$CONTROLLER"; do
    if [ ! -f "$F" ]; then
        echo "    MISSING: $F"
        MISSING=1
    fi
...
pkill -9 -f 'xapp_handover_gru'   2>/dev/null || true
...
echo ""
echo "[5/5] Triggering GRU simulation (LAUNCH ALL)..."
RESP=$(curl -s -X POST http://localhost:8001/ctrl/launch-all \
    -H "Content-Type: application/json" \
    -d "{\"scenario\":\"gru_scenario\",\"n_ues\":${N_UES},\"n_mmwave\":${N_CELLS},\"sim_time\":${SIM_TIME}}")
...
echo "║    tail -f /tmp/farouk_xapp.log   (GRU C xApp)               ║"
echo "║    tail -f /tmp/farouk_gru.log    (GRU Python service)       ║"
echo "║    tail -f /tmp/flexric.log       (FlexRIC E2 activity)      ║"
echo "║                                                              ║"
echo "║  Results auto-saved when done:                               ║"
echo "║    /home/omar_farouk/open-ran-clean/3D_GUI_Sim_Results/      ║"
echo "╚══════════════════════════════════════════════════════════════╝"
echo ""
