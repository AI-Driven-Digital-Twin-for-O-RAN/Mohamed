...
#define MAX_NUM_OF_RIC_INDICATIONS 3  // Number of measurements to average

// A3 Event Configuration (3GPP Standard)
#define A3_OFFSET 2.0           // dB - Offset for A3 event
#define A3_HYSTERESIS 0.5       // dB - Hysteresis to prevent ping-pong
#define A3_TIME_TO_TRIGGER 0    // Not implemented in this version

// Handover Timing Parameters
#define HANDOVER_COOLDOWN_SEC 5         // Cooldown between handovers for same UE
#define HANDOVER_COMPLETION_TIMEOUT 10   // Max time to wait for handover completion
#define MEASUREMENT_UPDATE_INTERVAL 5    // Seconds between decision cycles
...
typedef struct {
    ...
    time_t last_handover_time;    // For cooldown tracking
...
void add_neighCell(struct SINRServingValues* UE, const uint16_t neighCellID, const double sinr) {
    ...
    // Update existing neighbor with running average
        ...
        if (neighCell->measurement_count < MAX_NUM_OF_RIC_INDICATIONS) {
            // Running average
            neighCell->sinr = ((neighCell->sinr * neighCell->measurement_count) + sinr) / 
                             (neighCell->measurement_count + 1);
            ...
            // Reset after enough samples
            neighCell->sinr = sinr;
            neighCell->measurement_count = 1;
            
            printf("[Measurement] Reset neighbor %d for UE %d: SINR=%.2f dB\n",
                   neighCellID, UE->ueID, sinr);
        }
    }
}
...
bool can_attempt_handover(struct SINRServingValues* ue) {
    ...
    switch (ue->ho_context.state) {
        ...
        case HO_STATE_COOLDOWN:
            ...
            if (difftime(now, ue->ho_context.last_handover_time) >= HANDOVER_COOLDOWN_SEC) {
                printf("[StateMgmt] UE %d: Cooldown expired, ready for new handover\n", ue->ueID);
                ue->ho_context.state = HO_STATE_IDLE;
                return true;
            }
            printf("[StateMgmt] UE %d: In cooldown (%.0fs remaining)\n", 
                   ue->ueID, HANDOVER_COOLDOWN_SEC - difftime(now, ue->ho_context.last_handover_time));
            return false;
...
void update_handover_state_on_success(struct SINRServingValues* ue) {
    ...
    printf("[StateMgmt] UE %d: Handover #%d completed successfully\n",
           ue->ueID, ue->ho_context.total_handovers);
...
void confirm_handover_completion(uint16_t ueID, uint16_t old_cell, uint16_t new_cell) {
    ...
    if (old_context.state == HO_STATE_WAITING_COMPLETION &&
        old_context.target_cell == new_cell) {
        
        // ✅ Handover completed successfully!
        update_handover_state_on_success(new_ue);
        
    } else if (old_context.state == HO_STATE_WAITING_COMPLETION) {
...
bool execute_handover(e2_node_arr_xapp_t const* nodes, uint16_t ueID, 
    ...
    for (size_t i = 0; i < nodes->len; ++i) {
        ...
        if (ans.success) {
            ...
            printf("[HandoverExec] ✓ Command sent to node %zu\n", i);
        } else {
            printf("[HandoverExec] ✗ Failed to send to node %zu\n", i);
        }
    }
    ...
    if (handover_sent) {
        // ✅ FIX #1: DON'T mark as success yet - wait for KPM confirmation
    ...
    for (int cell_idx = 0; cell_idx < MAX_REGISTERED_CELLS; cell_idx++) {
        ...
        for (int ue_idx = 0; ue_idx < MAX_REGISTERED_UES; ue_idx++) {
            ...
            if (a3_result.a3_triggered) {
                ...
                // Execute handover
                bool success = execute_handover(nodes, ue->ueID, cell->cellID, 
...
int main(int argc, char* argv[]) {
    ...
    printf("╔══════════════════════════════════════════════════════════════╗\n");
    printf("║                                                              ║\n");
    printf("║          SINR-BASED HANDOVER xAPP - FIXED VERSION           ║\n");
    printf("║          ✅ Proper Multiple Handover Support                 ║\n");
    printf("║                                                              ║\n");
    printf("╚══════════════════════════════════════════════════════════════╝\n\n");
    ...
    printf("[Init] ✅ FIXES APPLIED:\n");
    printf("  1. Handover confirmation via KPM detection\n");
    printf("  2. Proper data structure updates after handover\n");
    printf("  3. No premature success marking\n\n");
    printf("[Init] Configuration:\n");
    printf("  - A3 Offset: %.1f dB\n", A3_OFFSET);
    printf("  - A3 Hysteresis: %.1f dB\n", A3_HYSTERESIS);
    printf("  - Handover Cooldown: %d seconds\n", HANDOVER_COOLDOWN_SEC);
