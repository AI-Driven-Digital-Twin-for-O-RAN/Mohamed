...
#  Usage:  bash kill_sim.sh   (from the repo root)
# ══════════════════════════════════════════════════════════════════════
...
> "$RUNTIME_CSV_HOME/kpm_handover_features.csv" 2>/dev/null \
...
LATEST_SIM=$(ls -td "${PROJECT_ROOT}/sim-results"/sim* 2>/dev/null | head -1)
if [ -n "$LATEST_SIM" ]; then
    SIM_NAME=$(basename "$LATEST_SIM")
    # Standard plots (InfluxDB / 2D charts)
    if [ ! -d "$LATEST_SIM/plots" ]; then
        python3 "${PROJECT_ROOT}/platform/controller/generate_plots.py" "$LATEST_SIM" \
    ...
    python3 "${PROJECT_ROOT}/platform/controller/generate_gru_pdf.py" "$LATEST_SIM" \
        > /dev/null 2>&1 \
